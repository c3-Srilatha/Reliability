...
describe(filename, function () {
  ...
  describe('#fetchDependentMetrics', function () {
    it('Returns correct fetch result of dependent metrics without limit and offset.', function () {
      this.spec = FetchSpec.make({
        include: 'name, description, expression, assetsDisplayStr, permission, lastUpdated',
      });
      this.result = ReliabilityMetric.fetchDependentMetrics('CustomTestMetric2', this.spec);

      expect(this.result.count).toEqual(1);
      expect(this.result.totalCount).toEqual(1);
      expect(this.result.objs[0].id).toEqual('CustomTestMetric4');
    });

    it('Returns correct fetch result of dependent metrics with limit and offset.', function () {
      this.spec = FetchSpec.make({
        include: 'name, description, expression, assetsDisplayStr, permission, lastUpdated',
        limit: 1,
        offset: 0,
    ...
    it('Returns correct fetch result of dependent metrics with limit, offset, and filter.', function () {
      ...
      expect(this.result.count).toEqual(0);
      expect(this.result.totalCount).toEqual(0);
    });
  });
});
