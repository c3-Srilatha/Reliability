// TypeScript definitions for the C3 type Pkg.Store.Config

/**
 * Config to access different pkgs from all backing stores Pkg.Store.
 * @see Pkg.Store
 *
 * @remarks this represents a value passed to a method that expects an instance of Pkg.Store.Config
 */
declare namespace Pkg.Store {
  export interface IConfig {
    ...
    /**
     * If true, Pkg can be constructed from Pkg.Store using `Pkg.Store.inst().pkg('testPkg') -> Pkg`
     */
    enabled?: boolean;

    /**
     * listed stores will be processed to access pkgs in the same order and first one wins,
     * if a pkg is available in more than one store.
     */
    stores?: C3.Array<Pkg.Store.Nested | null> | Array<IPkg.Store.Nested | null>;

    /**
     * git Workspace  info to pull and push for writable pkgs in Pkg.Store. @see configWithGit to set it.
     */
    gitWorkspace?: string | null;

    /**
     * If true, other app nodes will not be notified of Pkg.File.Event and Pkg.Store on other nodes will not invalidate
     * any pkg caches.
     */
    multiNodeUpdateDisabled?: boolean;

    /**
     * If true, run full UI Metadata validation for all UI metadata files.
     * If false, run normal validation on UI Metadata files.
     */
    fullUiMetadataValidationEnabled?: boolean;

    /**
     * If true, validate all types for runtime code-gen during package validation
     * If false, don't validate for runtime code-gen. The default is false
     */
    needRuntimeCodeValidation?: boolean;

    /**
     * Max concurrent valdiations queue processors.  Range (1, cores/2).
     */
    maxValidationQueueProcessors?: number | null;

    /**
     * @private
     * Number of paths to validate exceed this value, then will check to see if validation by pkg
     * needs to be done using fullValidationPct.
     * For testing path only, give high number to do avoid by pkg.
     */
    maxPathsToValidate?: number | null;

    /**
     * @private
     * If number of paths out of total paths in a Pkg to validate is more than this percent, full pkg will be validated.
     * otherwise, provided paths will be validated.
     */
    fullValidationPct?: number | null;

    /**
     * Semantic Version of the most recently loaded packages from ArtifactHub.
     */
    basePkgVersion?: string | null;

    /**
     * If true, dependency resolution will not be retried ignoring locks if a resolution error is thrown. Recommended
     * only to be true during CI or validation of locks. For this config to take effect please make sure {@link Pkg.Store.WriteSpec#ignoreLocks} is false
     */
    doNotIgnoreLocksUponResolutionError?: boolean;
  }
}
 ...
 * @see Pkg.Store
 *
 * @remarks this represents a made instance of Pkg.Store.Config
 */
declare namespace Pkg.Store {
  export class Config extends Obj {

    readonly configOverride?: string | null;
    withConfigOverride(configOverride: string | null): Pkg.Store.Config;

    readonly secretOverride?: string | null;
    withSecretOverride(secretOverride: string | null): Pkg.Store.Config;

    /**
     * Captures any issues that occurred while deserializing from filesystem
     */
    readonly issues?: C3.Array<string | null>;
    withIssues(issues: C3.Array<string | null> | Array<string | null>): Pkg.Store.Config;

    /**
     * The unique and required name of this instance.
     */
    readonly name: string;
    withName(name: string): Pkg.Store.Config;

    /**
     * If true, Pkg can be constructed from Pkg.Store using `Pkg.Store.inst().pkg('testPkg') -> Pkg`
     */
    readonly enabled?: boolean;
    withEnabled(enabled: boolean): Pkg.Store.Config;

    /**
     * listed stores will be processed to access pkgs in the same order and first one wins,
     * if a pkg is available in more than one store.
     */
    readonly stores?: C3.Array<Pkg.Store.Nested | null>;
    withStores(stores: C3.Array<Pkg.Store.Nested | null> | Array<IPkg.Store.Nested | null>): Pkg.Store.Config;

    /**
     * git Workspace  info to pull and push for writable pkgs in Pkg.Store. @see configWithGit to set it.
     */
    readonly gitWorkspace?: string | null;
    withGitWorkspace(gitWorkspace: string | null): Pkg.Store.Config;

    /**
     * If true, other app nodes will not be notified of Pkg.File.Event and Pkg.Store on other nodes will not invalidate
     * any pkg caches.
     */
    readonly multiNodeUpdateDisabled?: boolean;
    withMultiNodeUpdateDisabled(multiNodeUpdateDisabled: boolean): Pkg.Store.Config;

    /**
     * If true, run full UI Metadata validation for all UI metadata files.
     * If false, run normal validation on UI Metadata files.
     */
    readonly fullUiMetadataValidationEnabled?: boolean;
    withFullUiMetadataValidationEnabled(fullUiMetadataValidationEnabled: boolean): Pkg.Store.Config;

    /**
     * If true, validate all types for runtime code-gen during package validation
     * If false, don't validate for runtime code-gen. The default is false
     */
    readonly needRuntimeCodeValidation?: boolean;
    withNeedRuntimeCodeValidation(needRuntimeCodeValidation: boolean): Pkg.Store.Config;

    /**
     * Max concurrent valdiations queue processors.  Range (1, cores/2).
     */
    readonly maxValidationQueueProcessors?: number | null;
    withMaxValidationQueueProcessors(maxValidationQueueProcessors: number | null): Pkg.Store.Config;
     ...
     * For testing path only, give high number to do avoid by pkg.
     */
    readonly maxPathsToValidate?: number | null;
    withMaxPathsToValidate(maxPathsToValidate: number | null): Pkg.Store.Config;
     ...
     * otherwise, provided paths will be validated.
     */
    readonly fullValidationPct?: number | null;
    withFullValidationPct(fullValidationPct: number | null): Pkg.Store.Config;

    /**
     * Semantic Version of the most recently loaded packages from ArtifactHub.
     */
    readonly basePkgVersion?: string | null;
    withBasePkgVersion(basePkgVersion: string | null): Pkg.Store.Config;

    /**
     * If true, dependency resolution will not be retried ignoring locks if a resolution error is thrown. Recommended
     * only to be true during CI or validation of locks. For this config to take effect please make sure {@link Pkg.Store.WriteSpec#ignoreLocks} is false
     */
    readonly doNotIgnoreLocksUponResolutionError?: boolean;
    withDoNotIgnoreLocksUponResolutionError(doNotIgnoreLocksUponResolutionError: boolean): Pkg.Store.Config;
     ...
     * @see #toJson
     */
    static fromJson(json: any | null): Pkg.Store.Config | null;
     ...
     * @see #toJsonString
     */
    static fromJsonString(json: string | null): Pkg.Store.Config | null;
     ...
     * @see #toXmlString
     */
    static fromXmlString(xml: string | null): Pkg.Store.Config | null;
     ...
     */
    static deserialize(contentStr: string | null, contentType: string): Pkg.Store.Config | null;
     ...
     */
    replaceType(old: Type, new_: Type): Pkg.Store.Config;
     ...
     */
    mapFieldValues(mapper: λBiFunction<FieldType, any, any>, convertValue?: boolean): Pkg.Store.Config;
     ...
     *           if true, attempt to convert the value to match the field's type
     */
    mapFieldValues(spec: ValueSpec, mapper: λBiFunction<FieldType, any, any>, convertValue?: boolean): Pkg.Store.Config;
     ...
     *           if true, attempt to convert the value to match the field's type
     */
    mapFieldValuesAsync(mapper: λBiFunction<FieldType, any, C3.Promise<any> | null>, convertValue?: boolean): C3.Promise<Pkg.Store.Config | null>;
     ...
     *           if true, attempt to convert the value to match the field's type
     */
    mapFieldValuesAsync(spec: ValueSpec, mapper: λBiFunction<FieldType | null, any, C3.Promise<any> | null>, convertValue?: boolean): C3.Promise<Pkg.Store.Config | null>;
     ...
     *           if true, attempt to convert the value to match the field's type
     */
    mapFieldValue(field?: FieldType | null, includeEmpty?: boolean, mapper: λFunction<any, any>, convertValue?: boolean): Pkg.Store.Config;
     ...
     *           if true, attempt to convert the value to match the field's type
     */
    mapRefs(mapper: λBiFunction<FieldType, Obj, Obj | null>, convertValue?: boolean): Pkg.Store.Config;
     ...
     *           if true, attempt to convert the value to match the field's type
     */
    mapRefs(includeEmpty: boolean, mapper: λBiFunction<FieldType, Obj, Obj | null>, convertValue?: boolean): Pkg.Store.Config;

    /**
     * Populates all missing default values and throws error if any constraint is violated.
     */
    validateObj(): Pkg.Store.Config;
     ...
     * @see #defaultField
     */
    withField(field: string, value: any, doNotConvert?: boolean): Pkg.Store.Config;
     ...
     * @see #defaultField
     */
    withField(field: FieldType, value: any, doNotConvert?: boolean): Pkg.Store.Config;
     ...
     * @return new Obj
     */
    withFields(fields: C3.Map<string | null, any> | null, doNotConvert?: boolean): Pkg.Store.Config;
     ...
     * @return new Obj
     */
    withFieldAtPath(path: string, value: any, doNotConvert?: boolean, doNotCreateIfMissing?: boolean): Pkg.Store.Config;
     ...
     * @see #withoutField
     */
    withoutFieldAtPath(path: string): Pkg.Store.Config;
     ...
     * @see #removeField
     */
    withoutField(field: string | null): Pkg.Store.Config;
     ...
     * @see #removeField
     */
    withoutField(field: FieldType | null): Pkg.Store.Config;
     ...
     */
    withoutFields(fields: C3.Array<string | null> | null): Pkg.Store.Config;
     ...
     * @return new Obj with removed fields
     */
    withoutFieldsByType(fields: C3.Array<FieldType | null> | null): Pkg.Store.Config;

    /**
     * @return a new Obj, removing the field types marked with annotation @config(secret=true) recursively
     */
    withoutSecretFields(): Pkg.Store.Config;
     ...
     */
    withDefaults(includeEmptyRefsWithDefaults?: boolean, defaultFields?: C3.Array<string | null>): Pkg.Store.Config;
     ...
     * @see #unsetField
     */
    defaultField(field: string): Pkg.Store.Config;
     ...
     * @see #unsetField
     */
    defaultField(field: FieldType): Pkg.Store.Config;
     ...
     * @see #removeField
     */
    unsetField(field: string): Pkg.Store.Config;
     ...
     * @see #removeField
     */
    unsetField(field: FieldType): Pkg.Store.Config;
     ...
     * @see #unsetField
     */
    removeField(field: string): Pkg.Store.Config;
     ...
     * @see #unsetField
     */
    removeField(field: FieldType): Pkg.Store.Config;
     ...
     */
    mergeObj(other: Obj | null, fieldPathMergeSpec?: C3.Map<string | null, string | null>): Pkg.Store.Config;
     ...
     * @return the new merged Obj
     */
    mergeObj(other: Obj | null, otherFieldsFilter: Type): Pkg.Store.Config;
     ...
     */
    mergeObj(other: Obj | null, deep?: boolean, merger: λQuadFunction<FieldPath | null, any, FieldPath | null, any, any>): Pkg.Store.Config;
     ...
     */
    mergeObj(other: Obj | null, merger: λQuadFunction<FieldType | null, any, FieldType | null, any, any>): Pkg.Store.Config;

    mergeJson(json: any | null): Pkg.Store.Config;
     ...
     */
    mergeChildren(deep?: boolean, objKey?: λFunction<Obj | null, any> | null, filter?: λPredicate<string> | null): Pkg.Store.Config;
     ...
     */
    sumObj(other: Obj | null, deep?: boolean): Pkg.Store.Config;
     ...
     */
    singletonArray(): C3.Array<Pkg.Store.Config | null>;

    /**
     * Creates an array of instances of this type.
     */
    static array(...elements: any[]): C3.Array<Pkg.Store.Config | null>;

    /**
     * Creates an array of instances of this type.
     */
    static arrayBuilder(): ArrayBuilder<Pkg.Store.Config | null> | null;
     ...
     * @return new array instance with this as only element.
     */
    singletonSet(): C3.Set<Pkg.Store.Config | null>;

    /**
     * Creates a set of instances of this type.
     */
    static setBuilder(): SetBuilder<Pkg.Store.Config | null> | null;

    /**
     * Create a map of string to elements of this type.
     */
    static mapBuilder(): MapBuilder<string | null, Pkg.Store.Config | null> | null;

    /**
     * Create a map with the given key type and elements of this type.
     */
    static mapBuilderOf(keyType: ValueType): MapBuilder<any, Pkg.Store.Config | null> | null;

    /**
     * @return new ObjBuilder with initial state set to fields of this instance.
     */
    toBuilder(): ObjBuilder<Pkg.Store.Config | null>;

    /**
     * @return new ObjBuilder of this instance.
     */
    static builder(): ObjBuilder<Pkg.Store.Config | null>;

    /**
     * Construct instance of this type from provided field values and options
     */
    static fromFields(fields: C3.Map<FieldType | null, any> | null, spec: Obj.MakeSpec | null): Pkg.Store.Config;
     ...
     * @see withDefaults
     */
    static fromFields(fields: C3.Map<FieldType | null, any | any | null> | null, withDefaults?: boolean): Pkg.Store.Config;

    /**
     * Construct instance of this type from provided field values and options
     */
    static make(fields: C3.Map<string | null, any> | null, spec: Obj.MakeSpec | null): Pkg.Store.Config;
     ...
     * @see withDefaults
     */
    static make(withDefaults?: boolean): Pkg.Store.Config;
     ...
     * @see withDefaults
     */
    static make(fields: C3.Map<string | null, any | any | null> | null, withDefaults?: boolean): Pkg.Store.Config;
     ...
     * @see withDefaults
     */
    static make(fields: any, withDefaults?: boolean): Pkg.Store.Config;
     ...
     * @see #fromString
     */
    static make(s: string | null): Pkg.Store.Config | null;

    /**
     * Construct an instance of this type from provided instance of a subtype or a "duck type".
     */
    static remake(other: Obj | null, failIfExtraOrInvalidFields?: boolean): Pkg.Store.Config;
     ...
     */
    static cachedEmptyInst(): Pkg.Store.Config;

    /**
     * Generate a stream of instances of this type. The stream is endless and will call #generateObj each time a new
     * value is read.
     */
    static generateObjs(spec?: Obj.GenerateSpec | null): Stream<Pkg.Store.Config>;
     ...
     */
    static generateObj(spec?: Obj.GenerateSpec | null): Pkg.Store.Config;
     ...
     */
    static allCached(doNotProduceAll?: boolean): Stream<Pkg.Store.Config | null>;
     ...
     */
    static find(filter?: string | null, doNotProduceAll?: boolean): Stream<Pkg.Store.Config | null>;
     ...
     * @return existing instances of this type that satisfy filter from data cache.
     */
    static findBy(field: string, value: any, doNotProduceAll?: boolean): Stream<Pkg.Store.Config | null>;
     ...
     * @see getCached
     */
    static findByCacheKey(key: string): Pkg.Store.Config | null;

    /**
     * Returns cached instance by key, with secrets removed.
     */
    static forCacheKey(key: string | null): Pkg.Store.Config | null;
     ...
     * @see findByCacheKey
     */
    getCached(): Pkg.Store.Config | null;

    /**
     * @return newly produced cached instance.
     */
    refreshCache(): Pkg.Store.Config | null;
     ...
     */
    static produce(key: string): Pkg.Store.Config | null;
     ...
     */
    static produceAll(): C3.Map<string | null, Pkg.Store.Config | null>;

    /**
     * Override for sub-types to produce all values.
     */
    static doProduceAll(): C3.Map<string | null, Pkg.Store.Config | null>;
     ...
     */
    static forConfigKey(configKey: string | null): Pkg.Store.Config | null;
     ...
     */
    static listConfigs(parallel?: boolean, filter?: λPredicate<string> | null): Stream<Pkg.Store.Config | null>;
     ...
     * @see setConfigValue
     */
    setConfigValues(from: Pkg.Store.Config, override?: string | null): void;
     ...
     * @see setSecretValue
     */
    setSecretValues(from: Pkg.Store.Config, override?: string | null): void;
     ...
     */
    static loadConfigAndSecret(configKey: string): Pkg.Store.Config | null;
     ...
     * @see #toString
     */
    static fromString(s: string | null): Pkg.Store.Config | null;
     ...
     */
    static forName(name: string | null, failIfMissing?: boolean): Pkg.Store.Config | null;
  }
}
