// TypeScript definitions for the C3 type CalcFieldsQueue

/**
 * Invalidation queue for stored calc processing.  Each entry is for invalidating one or more calc fields in one target
 * instance.
 *
 * @see CalcFieldsQueueEntry
 *
 * @remarks this represents a value passed to a method that expects an instance of CalcFieldsQueue
 */
declare interface ICalcFieldsQueue {
}
 ...
 * @see CalcFieldsQueueEntry
 *
 * @remarks this represents a made instance of CalcFieldsQueue
 */
declare class CalcFieldsQueue {
   ...
   */
  static compute(targetType: Type, entries: C3.Array<CalcFieldsQueueEntry | null> | null): void;
   ...
   */
  static find(spec?: InvalidationQueue.FindSpec | null): C3.Array<CalcFieldsQueueEntry | null>;
   ...
   * @return the list of requested entries.
   */
  static findStream(spec?: InvalidationQueue.FindStreamSpec | null): Stream<CalcFieldsQueueEntry | null>;
   ...
   *           Entries to remove.
   */
  static clearEntries(entries: C3.Array<CalcFieldsQueueEntry | null> | null): void;
   ...
   * This function should be claimed by individual InvalidationQueue types such as CalcFieldsQueue.
   */
  static mergeContexts(targetType?: Type | null, entries?: C3.Array<CalcFieldsQueueEntry | null>): C3.Array<CalcFieldsQueueEntry | null>;

  /**
   * Scans the queue, grouping by targetTypeId/context and, for every entry where the threshold is
   * reached, schedules a refreschCalcFields call in the action queue for the queue entries and the fields specified in
   * the context.  The calc field queue entries will be removed during the refreshCalcFields operation.
   *
   *
   * @param thresholdPct
   *           Percentage of entries (between 1 and 100) that should exist for a particular
   *           targetTypeId/context in comparison to the total number of entries in that target type
   *           that should trigger rescheduling.  A value of <= 0 will process all entries/types.  Default is 50%
   * @param batchSize
   *           Batch size to use for calls to refreshCalcFields.
   */
  static rescheduleByType(thresholdPct?: number | null, batchSize?: number | null): void;
}
