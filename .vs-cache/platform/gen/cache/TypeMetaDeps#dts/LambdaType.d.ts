// TypeScript definitions for the C3 type LambdaType

/**
 * Value type for `lambda` i.e. function as a value. The structure of the lambda signature comes from
 * {@link FunctionType}. The implementation carried as data is what makes it a **lambda**.
 *
 * Note that lambdas are _data values_ and thus a type field declared as a lambda is not defined by that type itself,
 * but will be a data field (with a {@link FieldType}). The field value may be null or may be supplied with a
 * lambda instance.
 *
 * ```type
 * type X {
 *   method: member function(a: !int): !int
 *   lambda: lambda(a: !int): !int
 * }
 * ```
 *
 * In the example above, **method** and **lambda** are both declared in the type, however, they are treated differently
 * because the first one is a {@link Method} and the second is a lambda. Instances of the Obj will have only a single
 * field, for **lambda**, and it will only be set if there is a value supplied. All instances will share the same
 * **method** and it will be implemented on the type itself.
 *
 * ```js
 * let x = X.make({ lambda: a => a * 2 })
 * x.method(3)
 * x.lambda(3)
 * ```
 *
 * ```py
 * x = c3.X(lambda_ = lambda a: a * 2)
 * x.method(3)
 * x.lambda(3)
 * ```
 *
 * So in the above examples, both end up being callable from the instances, but the method implementation goes with
 * the type while the lambda implementation is a field value that goes with the instance.
 *
 * ```java
 * X x = X.builder().lambda(a -> a * 2).build();
 * x.method(3);
 * x.lambda().apply(3);
 * ```
 *
 * In Java, the lambdas are strongly typed (using the
 * [FunctionalInterface](https://docs.oracle.com/javase/8/docs/api/java/lang/FunctionalInterface.html) annotation),
 * so must be invoked using the correct method name. `apply` is appropriate here since this lambda will be a
 * [Function<Long, Long>](https://docs.oracle.com/javase/8/docs/api/java/util/function/Function.html).
 *
 * @see MethodType
 * @see methods.c3doc#lambdas
 *
 * @remarks this represents a value passed to a method that expects an instance of LambdaType
 */
declare interface ILambdaType {
 ...
 * @see methods.c3doc#lambdas
 *
 * @remarks this represents a made instance of LambdaType
 */
declare class LambdaType extends Obj {

  /**
   * Source file Pkg.Path a C3 Type declaration [.c3typ].
   */
  readonly declaredPkgPath?: string | null;
  withDeclaredPkgPath(declaredPkgPath: string | null): LambdaType;

  /**
   * Source line number in a C3 Type declaration [.c3typ] file starting with 1 where this metadata element is declared.
   */
  readonly declaredSrcLine?: number | null;
  withDeclaredSrcLine(declaredSrcLine: number | null): LambdaType;
  ...
  withDeclaredSrcCharInLine(declaredSrcCharInLine: number | null): LambdaType;
   ...
   */
  readonly pkg: Pkg;
  withPkg(pkg: IPkg): LambdaType;

  /**
   * SDK json representing instance of this type
   */
  readonly json?: any;
  withJson(json: any): LambdaType;

  /**
   * The set of annotations defined on this element of metadata represented as json value.
   */
  readonly annotationsJson?: any | null;
  withAnnotationsJson(annotationsJson: any | null): LambdaType;
  ...
  withModifier(modifier: string | null): LambdaType;

  readonly name?: string | null;
  withName(name: string | null): LambdaType;
  ...
  withVarBindings(varBindings: C3.Map<string | null, ValueType | null> | {[key: string | null]: IValueType | null}): LambdaType;

  /**
   * The list of variable declarations and any restrictions on them.
   */
  readonly genericVars?: C3.Array<VarReferenceType | null>;
  withGenericVars(genericVars: C3.Array<VarReferenceType | null> | Array<IVarReferenceType | null>): LambdaType;

  /**
   * The formal parameters accepted by this function.
   */
  readonly params?: C3.Array<FunctionParam | null>;
  withParams(params: C3.Array<FunctionParam | null> | Array<IFunctionParam | null>): LambdaType;

  /**
   * The return type of the function.
   */
  readonly returnType?: ValueType | null;
  withReturnType(returnType: IValueType | null): LambdaType;
  ...
  withStateful(stateful: boolean): LambdaType;
   ...
   */
  replaceType(old: Type, new_: Type): LambdaType;
   ...
   * @see #toJson
   */
  static fromJson(json: any | null): LambdaType | null;
   ...
   * @see #toJsonString
   */
  static fromJsonString(json: string | null): LambdaType | null;
   ...
   * @see #toXmlString
   */
  static fromXmlString(xml: string | null): LambdaType | null;
   ...
   */
  static deserialize(contentStr: string | null, contentType: string): LambdaType | null;
   ...
   */
  mapFieldValues(mapper: λBiFunction<FieldType, any, any>, convertValue?: boolean): LambdaType;
   ...
   *           if true, attempt to convert the value to match the field's type
   */
  mapFieldValues(spec: ValueSpec, mapper: λBiFunction<FieldType, any, any>, convertValue?: boolean): LambdaType;
   ...
   *           if true, attempt to convert the value to match the field's type
   */
  mapFieldValuesAsync(mapper: λBiFunction<FieldType, any, C3.Promise<any> | null>, convertValue?: boolean): C3.Promise<LambdaType | null>;
   ...
   *           if true, attempt to convert the value to match the field's type
   */
  mapFieldValuesAsync(spec: ValueSpec, mapper: λBiFunction<FieldType | null, any, C3.Promise<any> | null>, convertValue?: boolean): C3.Promise<LambdaType | null>;
   ...
   *           if true, attempt to convert the value to match the field's type
   */
  mapFieldValue(field?: FieldType | null, includeEmpty?: boolean, mapper: λFunction<any, any>, convertValue?: boolean): LambdaType;
   ...
   *           if true, attempt to convert the value to match the field's type
   */
  mapRefs(mapper: λBiFunction<FieldType, Obj, Obj | null>, convertValue?: boolean): LambdaType;
   ...
   *           if true, attempt to convert the value to match the field's type
   */
  mapRefs(includeEmpty: boolean, mapper: λBiFunction<FieldType, Obj, Obj | null>, convertValue?: boolean): LambdaType;

  /**
   * Populates all missing default values and throws error if any constraint is violated.
   */
  validateObj(): LambdaType;
   ...
   * @see #defaultField
   */
  withField(field: string, value: any, doNotConvert?: boolean): LambdaType;
   ...
   * @see #defaultField
   */
  withField(field: FieldType, value: any, doNotConvert?: boolean): LambdaType;
   ...
   * @return new Obj
   */
  withFields(fields: C3.Map<string | null, any> | null, doNotConvert?: boolean): LambdaType;
   ...
   * @return new Obj
   */
  withFieldAtPath(path: string, value: any, doNotConvert?: boolean, doNotCreateIfMissing?: boolean): LambdaType;
   ...
   * @see #withoutField
   */
  withoutFieldAtPath(path: string): LambdaType;
   ...
   * @see #removeField
   */
  withoutField(field: string | null): LambdaType;
   ...
   * @see #removeField
   */
  withoutField(field: FieldType | null): LambdaType;
   ...
   */
  withoutFields(fields: C3.Array<string | null> | null): LambdaType;
   ...
   * @return new Obj with removed fields
   */
  withoutFieldsByType(fields: C3.Array<FieldType | null> | null): LambdaType;

  /**
   * @return a new Obj, removing the field types marked with annotation @config(secret=true) recursively
   */
  withoutSecretFields(): LambdaType;
   ...
   */
  withDefaults(includeEmptyRefsWithDefaults?: boolean, defaultFields?: C3.Array<string | null>): LambdaType;
   ...
   * @see #unsetField
   */
  defaultField(field: string): LambdaType;
   ...
   * @see #unsetField
   */
  defaultField(field: FieldType): LambdaType;
   ...
   * @see #removeField
   */
  unsetField(field: string): LambdaType;
   ...
   * @see #removeField
   */
  unsetField(field: FieldType): LambdaType;
   ...
   * @see #unsetField
   */
  removeField(field: string): LambdaType;
   ...
   * @see #unsetField
   */
  removeField(field: FieldType): LambdaType;
   ...
   */
  mergeObj(other: Obj | null, fieldPathMergeSpec?: C3.Map<string | null, string | null>): LambdaType;
   ...
   * @return the new merged Obj
   */
  mergeObj(other: Obj | null, otherFieldsFilter: Type): LambdaType;
   ...
   */
  mergeObj(other: Obj | null, deep?: boolean, merger: λQuadFunction<FieldPath | null, any, FieldPath | null, any, any>): LambdaType;
   ...
   */
  mergeObj(other: Obj | null, merger: λQuadFunction<FieldType | null, any, FieldType | null, any, any>): LambdaType;

  mergeJson(json: any | null): LambdaType;
   ...
   */
  mergeChildren(deep?: boolean, objKey?: λFunction<Obj | null, any> | null, filter?: λPredicate<string> | null): LambdaType;
   ...
   */
  sumObj(other: Obj | null, deep?: boolean): LambdaType;
   ...
   */
  singletonArray(): C3.Array<LambdaType | null>;

  /**
   * Creates an array of instances of this type.
   */
  static array(...elements: any[]): C3.Array<LambdaType | null>;

  /**
   * Creates an array of instances of this type.
   */
  static arrayBuilder(): ArrayBuilder<LambdaType | null> | null;
   ...
   * @return new array instance with this as only element.
   */
  singletonSet(): C3.Set<LambdaType | null>;

  /**
   * Creates a set of instances of this type.
   */
  static setBuilder(): SetBuilder<LambdaType | null> | null;

  /**
   * Create a map of string to elements of this type.
   */
  static mapBuilder(): MapBuilder<string | null, LambdaType | null> | null;

  /**
   * Create a map with the given key type and elements of this type.
   */
  static mapBuilderOf(keyType: ValueType): MapBuilder<any, LambdaType | null> | null;

  /**
   * @return new ObjBuilder with initial state set to fields of this instance.
   */
  toBuilder(): ObjBuilder<LambdaType | null>;

  /**
   * @return new ObjBuilder of this instance.
   */
  static builder(): ObjBuilder<LambdaType | null>;

  /**
   * Construct instance of this type from provided field values and options
   */
  static fromFields(fields: C3.Map<FieldType | null, any> | null, spec: Obj.MakeSpec | null): LambdaType;
   ...
   * @see withDefaults
   */
  static fromFields(fields: C3.Map<FieldType | null, any | any | null> | null, withDefaults?: boolean): LambdaType;

  /**
   * Construct instance of this type from provided field values and options
   */
  static make(fields: C3.Map<string | null, any> | null, spec: Obj.MakeSpec | null): LambdaType;
   ...
   * @see withDefaults
   */
  static make(withDefaults?: boolean): LambdaType;
   ...
   * @see withDefaults
   */
  static make(fields: C3.Map<string | null, any | any | null> | null, withDefaults?: boolean): LambdaType;
   ...
   * @see withDefaults
   */
  static make(fields: any, withDefaults?: boolean): LambdaType;
   ...
   * @see #fromString
   */
  static make(s: string | null): LambdaType | null;

  /**
   * Construct an instance of this type from provided instance of a subtype or a "duck type".
   */
  static remake(other: Obj | null, failIfExtraOrInvalidFields?: boolean): LambdaType;
   ...
   */
  static cachedEmptyInst(): LambdaType;

  /**
   * Generate a stream of instances of this type. The stream is endless and will call #generateObj each time a new
   * value is read.
   */
  static generateObjs(spec?: Obj.GenerateSpec | null): Stream<LambdaType>;
   ...
   */
  static generateObj(spec?: Obj.GenerateSpec | null): LambdaType;
   ...
   */
  withoutDeclaredSrcPos(recursive?: boolean): LambdaType;
   ...
   */
  withAnnotation(ann: Annotation): LambdaType;
   ...
   *         value for the field
   */
  withAnnotation(ann: string, annField?: string | null, value?: any): LambdaType;
   ...
   * @see #toString
   */
  static fromString(s: string | null): LambdaType | null;
   ...
   * @return guessed type or null if specific type cannot be determined
   */
  static guessTypeFromJavaClass(value: any, failIfNone?: boolean): LambdaType | null;
   ...
   */
  paramDefaultValue(param: string): any;
  ...
  /**
   * @return true if this lambda can be represented as Java Functional Interface.
   */
  isJavaNative(): boolean;

  /**
   * Create a consumer lambda type that takes zero or more parameters.
   */
  static consumer(...params: ValueType | null[]): LambdaType;

  /**
   * Create a function lambda type that takes zero or more parameters and returns a value. The last argument
   * to this function becomes the return type.
   */
  static function(...paramsAndRet: ValueType | null[]): LambdaType;

  /**
   * Create a predicate lambda type that takes zero or more parameters and returns a boolean value.
   */
  static predicate(...params: ValueType | null[]): LambdaType;

  /**
   * Create a supplier lambda type that takes no parameters and returns a value. The argument to this function becomes
   * the return type.
   */
  static supplier(ret: ValueType): LambdaType;

  /**
   * lambda(any, any)
   */
  static biConsumerOfAny(): LambdaType;

  /**
   * lambda(any, any): any
   */
  static biFunctionOfAny(): LambdaType;

  /**
   * lambda(any, any): boolean
   */
  static biPredicateOfAny(): LambdaType;

  /**
   * lambda(): any
   */
  static callableOfAny(): LambdaType;

  /**
   * lambda(any)
   */
  static consumerOfAny(): LambdaType;

  /**
   * lambda(any): any
   */
  static functionOfAny(): LambdaType;

  /**
   * lambda(any): boolean
   */
  static predicateOfAny(): LambdaType;

  /**
   * lambda(any, any, any, any)
   */
  static quadConsumerOfAny(): LambdaType;

  /**
   * lambda(any, any, any, any): any
   */
  static quadFunctionOfAny(): LambdaType;

  /**
   * lambda(any, any, any, any): boolean
   */
  static quadPredicateOfAny(): LambdaType;

  /**
   * lambda()
   */
  static runnable(): LambdaType;

  /**
   * lambda(): any
   */
  static supplierOfAny(): LambdaType;

  /**
   * lambda(any, any, any)
   */
  static triConsumerOfAny(): LambdaType;

  /**
   * lambda(any, any, any): any
   */
  static triFunctionOfAny(): LambdaType;

  /**
   * lambda(any, any, any): boolean
   */
  static triPredicateOfAny(): LambdaType;

  static ofAnyFunction(): LambdaType;

  /**
   * @return a list of the possible `LambdaConverter` for native java lambdas for this LambdaType.
   */
  nativeJavaLambdaConverters(): any;
}
