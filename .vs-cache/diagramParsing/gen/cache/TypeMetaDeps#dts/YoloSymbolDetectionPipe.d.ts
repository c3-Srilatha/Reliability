// TypeScript definitions for the C3 type YoloSymbolDetectionPipe

/**
 * This type represents an {@link MlPipe} template for YOLOv7 symbol detection in diagrams.
 *
 * @remarks this represents a value passed to a method that expects an instance of YoloSymbolDetectionPipe
 */
declare interface IYoloSymbolDetectionPipe {
  ...
  /**
   * The name of the column containing the urls of the images to be processed.
   */
  imageColumnName: string;

  /**
   * A list of {@link DiagramAugmenter}s to be used, such as {@link DiagramRandomCropper}. This field is required
   * if the user wants to train the pipe using {@link train}.
   */
  augmenters?: Array_Type<DiagramAugmenter> | Array<IDiagramAugmenter>;

  /**
   * Inference size (pixels)
   * An original image (with essential padding) would be split into multiple (possibly overlapped patches).
   * The size of each image patch is (imgSize, imgSize).
   * Larger values generally result in more accurate but slower detections.
   */
  imgSize: number;

  /**
   * Overlap ratio of two adjacent tiles, 0 <= overlapRatio <= 1
   * The degree of overlap between adjacent tiles when splitting the input image for processing.
   * A higher value can improve detection but might increase processing time.
   */
  overlapRatio: number;

  /**
   * Don't trace model
   * If set to true, the model won't be traced during inference.
   * If set to false, a dynamic PyTorch model will be converted into a static TorchScript representation.
   * This conversion involves tracing the model's execution to capture its control flow and operations as a static computation graph.
   * Tracing can improve performance but might limit dynamic model behavior.
   */
  noTrace: boolean;

  /**
   * Augmented inference
   * If true, augmented inference techniques are applied during detection.
   * This means that the same data augmentation techniques are applied to the input image or patch during inference time. This results in multiple augmented versions of the input image.
   * Augmentation can improve detection robustness but may also increase computation time.
   */
  augment: boolean;

  /**
   * Object confidence threshold
   * The minimum confidence level required for an object to be detected.
   * Objects with confidence scores below this threshold will be disregarded.
   */
  confThres: number;
   ...
   * A higher value can prevent multiple detections of the same object, but might also filter out some valid ones.
   */
  iouThres: number;

  /**
   * Filter by class: --class ['TAG'], or --class ['TAG', 'LMI']
   * An array of class indices to filter the detected objects.
   * If specified, only objects belonging to these classes will be included in the results.
   */
  filteredClasses?: Array_Type<string> | Array<string>;

  /**
   * Class-agnostic NMS
   * If set to true, class information is not considered during NMS, and only spatial overlap is used for filtering detections.
   * If set to false, for each class detected by the model, NMS is used to remove redundant bounding boxes within that class.
   * As a result, each class may have multiple detections, but within each class, duplicate detections are suppressed.
   */
  agnosticNMS: boolean;

  /**
   * Cuda device, i.e. 0 or 0,1,2,3 or cpu
   * Specifies the CUDA device to be used for inference.
   * You can set it to a specific GPU device (e.g., "0" or "0,1,2,3") or "cpu" for CPU inference.
   */
  device: string;

  /**
   * Target symbols that the detector can detect
   * A list of symbols or classes that the object detector is trained to recognize.
   * Detection will be limited to these specified targets.
   */
  targets: Array_Type<string> | Array<string>;

  /**
   * Trained pytorch state_dict file
   * The file containing the trained PyTorch model's state dictionary.
   * This file is necessary for loading the pre-trained model.
   */
  pth: IFile;

  /**
   * Yaml model file
   * The YAML file containing the model's configuration.
   * It provides information about the network architecture, hyperparameters, and class labels.
   */
  yaml: IFile;
}
 ...
 * This type represents an {@link MlPipe} template for YOLOv7 symbol detection in diagrams.
 *
 * @remarks this represents a made instance of YoloSymbolDetectionPipe
 */
declare class YoloSymbolDetectionPipe extends Obj  {
   ...
   */
  readonly id: string;
  withId(id: string): YoloSymbolDetectionPipe;
  ...
  withVersionEdits(versionEdits: Array_Type<VersionEdit> | Array<IVersionEdit> | null): YoloSymbolDetectionPipe;

  /**
   * Name of this pipe.
   */
  readonly name?: string;
  withName(name: string | null): YoloSymbolDetectionPipe;

  /**
   * Various system fields.
   */
  readonly meta?: Meta;
  withMeta(meta: IMeta | null): YoloSymbolDetectionPipe;
  ...
  withVersion(version: number | null): YoloSymbolDetectionPipe;

  /**
   * Persists concrete type with bindings for generic types where instance has parameter bindings
   */
  readonly typeWithBindings?: Type;
  withTypeWithBindings(typeWithBindings: IType | null): YoloSymbolDetectionPipe;
  ...
  withUserUpdatedFields(userUpdatedFields: Array_Type<string> | Array<string> | null): YoloSymbolDetectionPipe;
  ...
  withHidden(hidden: boolean): YoloSymbolDetectionPipe;
  ...
  withUserOwned(userOwned: boolean): YoloSymbolDetectionPipe;
  ...
  withTypeIdent(typeIdent: string | null): YoloSymbolDetectionPipe;

  /**
   * If true, then the pipe shall not be retrained
   */
  readonly frozen?: boolean;
  withFrozen(frozen: boolean): YoloSymbolDetectionPipe;

  /**
   * True if training has succeeded and false otherwise.
   */
  readonly trained?: boolean;
  withTrained(trained: boolean): YoloSymbolDetectionPipe;

  /**
   * Errors from training.
   */
  readonly trainingErrors?: string;
  withTrainingErrors(trainingErrors: string | null): YoloSymbolDetectionPipe;

  /**
   * If true, the underlying model only accepts flattened input.
   */
  readonly flattenInput?: boolean;
  withFlattenInput(flattenInput: boolean): YoloSymbolDetectionPipe;

  /**
   * The hyperparameters overridden of a native model or pipe.
   */
  readonly hyperparamOverrides?: Map_Type<string, any>;
  withHyperparamOverrides(hyperparamOverrides: Map_Type<string, any> | {[key: string]: any} | null): YoloSymbolDetectionPipe;
  ...
  withHyperparamSearchSpace(hyperparamSearchSpace: IHp.SearchSpace | null): YoloSymbolDetectionPipe;
  ...
  withHpoSpec(hpoSpec: IHpo.Spec | null): YoloSymbolDetectionPipe;

  /**
   * {@link Hpo.Experiment} stores the result of hyperparameter optimization to find the best model/pipeline.
   */
  readonly hpoExperiment?: Hpo.Experiment;
  withHpoExperiment(hpoExperiment: IHpo.Experiment | null): YoloSymbolDetectionPipe;

  /**
   * Reference to the experiment that produced this {@link MlPipe}. For example, this could be a {@link Hpo.ExperimentBase}.
   */
  readonly experiment?: any;
  withExperiment(experiment: any | null): YoloSymbolDetectionPipe;
  ...
  withModelFiles(modelFiles: Map_Type<string, Url> | {[key: string]: IUrl} | null): YoloSymbolDetectionPipe;
  ...
  withCustomModelCode(customModelCode: Map_Type<string, any> | {[key: string]: any} | null): YoloSymbolDetectionPipe;

  /**
   * Name for the function on the native model to call to do {@link MlProcessable#process}.
   */
  readonly processFunc?: string;
  withProcessFunc(processFunc: string | null): YoloSymbolDetectionPipe;
  ...
  withUnderlyingPipe(underlyingPipe: IMlAtomicPipe<any, any, any> | null): YoloSymbolDetectionPipe;

  /**
   * The name of the column containing the urls of the images to be processed.
   */
  readonly imageColumnName: string;
  withImageColumnName(imageColumnName: string): YoloSymbolDetectionPipe;

  /**
   * A list of {@link DiagramAugmenter}s to be used, such as {@link DiagramRandomCropper}. This field is required
   * if the user wants to train the pipe using {@link train}.
   */
  readonly augmenters?: Array_Type<DiagramAugmenter>;
  withAugmenters(augmenters: Array_Type<DiagramAugmenter> | Array<IDiagramAugmenter> | null): YoloSymbolDetectionPipe;
  ...
  withImgSize(imgSize: number): YoloSymbolDetectionPipe;
  ...
  withOverlapRatio(overlapRatio: number): YoloSymbolDetectionPipe;
  ...
  withNoTrace(noTrace: boolean): YoloSymbolDetectionPipe;
  ...
  withAugment(augment: boolean): YoloSymbolDetectionPipe;
  ...
  withConfThres(confThres: number): YoloSymbolDetectionPipe;
  ...
  withIouThres(iouThres: number): YoloSymbolDetectionPipe;
   ...
   * If specified, only objects belonging to these classes will be included in the results.
   */
  readonly filteredClasses?: Array_Type<string>;
  withFilteredClasses(filteredClasses: Array_Type<string> | Array<string> | null): YoloSymbolDetectionPipe;
   ...
   * As a result, each class may have multiple detections, but within each class, duplicate detections are suppressed.
   */
  readonly agnosticNMS: boolean;
  withAgnosticNMS(agnosticNMS: boolean): YoloSymbolDetectionPipe;
  ...
  withDevice(device: string): YoloSymbolDetectionPipe;
  ...
  withTargets(targets: Array_Type<string> | Array<string>): YoloSymbolDetectionPipe;
   ...
   */
  readonly pth: File;
  withPth(pth: IFile): YoloSymbolDetectionPipe;
   ...
   */
  readonly yaml: File;
  withYaml(yaml: IFile): YoloSymbolDetectionPipe;
   ...
   * @see #toJson
   */
  static fromJson(json: any): YoloSymbolDetectionPipe | null;
   ...
   * @see #toJsonString
   */
  static fromJsonString(json: string): YoloSymbolDetectionPipe | null;
   ...
   * @see #toXmlString
   */
  static fromXmlString(xml: string): YoloSymbolDetectionPipe | null;
   ...
   */
  static deserialize(contentStr: string, contentType: string): YoloSymbolDetectionPipe | null;
   ...
   */
  replaceType(old: Type, new_: Type): YoloSymbolDetectionPipe;
   ...
   */
  mapFieldValues(mapper: λBiFunction<FieldType, any, any | null>, convertValue?: boolean): YoloSymbolDetectionPipe;
   ...
   *           if true, attempt to convert the value to match the field's type
   */
  mapFieldValues(spec: ValueSpec, mapper: λBiFunction<FieldType, any, any | null>, convertValue?: boolean): YoloSymbolDetectionPipe;
   ...
   *           if true, attempt to convert the value to match the field's type
   */
  mapFieldValuesAsync(mapper: λBiFunction<FieldType, any, Promise<any> | null>, convertValue?: boolean): Promise<YoloSymbolDetectionPipe>;
   ...
   *           if true, attempt to convert the value to match the field's type
   */
  mapFieldValuesAsync(spec: ValueSpec, mapper: λBiFunction<FieldType | null, any, Promise<any> | null>, convertValue?: boolean): Promise<YoloSymbolDetectionPipe>;
   ...
   *           if true, attempt to convert the value to match the field's type
   */
  mapFieldValue(field?: FieldType, includeEmpty?: boolean, mapper: λFunction<any, any | null>, convertValue?: boolean): YoloSymbolDetectionPipe;
   ...
   *           if true, attempt to convert the value to match the field's type
   */
  mapRefs(mapper: λBiFunction<FieldType, Obj, Obj | null>, convertValue?: boolean): YoloSymbolDetectionPipe;
   ...
   *           if true, attempt to convert the value to match the field's type
   */
  mapRefs(includeEmpty: boolean, mapper: λBiFunction<FieldType, Obj, Obj | null>, convertValue?: boolean): YoloSymbolDetectionPipe;

  /**
   * Populates all missing default values and throws error if any constraint is violated.
   */
  validateObj(): YoloSymbolDetectionPipe;
   ...
   * @see #defaultField
   */
  withField(field: string, value: any, doNotConvert?: boolean): YoloSymbolDetectionPipe;
   ...
   * @see #defaultField
   */
  withField(field: FieldType, value: any, doNotConvert?: boolean): YoloSymbolDetectionPipe;
   ...
   * @return new Obj
   */
  withFields(fields: Map_Type<string, any>, doNotConvert?: boolean): YoloSymbolDetectionPipe;
   ...
   * @return new Obj
   */
  withFieldAtPath(path: string, value: any, doNotConvert?: boolean, doNotCreateIfMissing?: boolean): YoloSymbolDetectionPipe;
   ...
   * @see #withoutField
   */
  withoutFieldAtPath(path: string): YoloSymbolDetectionPipe;
   ...
   * @see #removeField
   */
  withoutField(field: string): YoloSymbolDetectionPipe;
   ...
   * @see #removeField
   */
  withoutField(field: FieldType): YoloSymbolDetectionPipe;
   ...
   */
  withoutFields(fields: Array_Type<string>): YoloSymbolDetectionPipe;
   ...
   * @return new Obj with removed fields
   */
  withoutFieldsByType(fields: Array_Type<FieldType>): YoloSymbolDetectionPipe;

  /**
   * @return a new Obj, removing the field types marked with annotation @config(secret=true) recursively
   */
  withoutSecretFields(): YoloSymbolDetectionPipe;
   ...
   */
  withDefaults(includeEmptyRefsWithDefaults?: boolean, defaultFields?: Array_Type<string>): YoloSymbolDetectionPipe;
   ...
   * @see #unsetField
   */
  defaultField(field: string): YoloSymbolDetectionPipe;
   ...
   * @see #unsetField
   */
  defaultField(field: FieldType): YoloSymbolDetectionPipe;
   ...
   * @see #removeField
   */
  unsetField(field: string): YoloSymbolDetectionPipe;
   ...
   * @see #removeField
   */
  unsetField(field: FieldType): YoloSymbolDetectionPipe;
   ...
   * @see #unsetField
   */
  removeField(field: string): YoloSymbolDetectionPipe;
   ...
   * @see #unsetField
   */
  removeField(field: FieldType): YoloSymbolDetectionPipe;
   ...
   */
  mergeObj(other: Obj, fieldPathMergeSpec?: Map_Type<string, string>): YoloSymbolDetectionPipe;
   ...
   * @return the new merged Obj
   */
  mergeObj(other: Obj, otherFieldsFilter: Type): YoloSymbolDetectionPipe;
   ...
   */
  mergeObj(other: Obj, deep?: boolean, merger: λQuadFunction<FieldPath | null, any | null, FieldPath | null, any | null, any | null>): YoloSymbolDetectionPipe;
   ...
   */
  mergeObj(other: Obj, merger: λQuadFunction<FieldType | null, any | null, FieldType | null, any | null, any | null>): YoloSymbolDetectionPipe;

  mergeJson(json: any): YoloSymbolDetectionPipe;
   ...
   */
  mergeChildren(deep?: boolean, objKey?: λFunction<Obj | null, any | null>, filter?: λPredicate<string>): YoloSymbolDetectionPipe;
   ...
   */
  sumObj(other: Obj, deep?: boolean): YoloSymbolDetectionPipe;
   ...
   */
  singletonArray(): Array_Type<YoloSymbolDetectionPipe>;

  /**
   * Creates an array of instances of this type.
   */
  static array(...elements: Array_Type<any>[]): Array_Type<YoloSymbolDetectionPipe> | null;

  /**
   * Creates an array of instances of this type.
   */
  static arrayBuilder(): ArrayBuilder<YoloSymbolDetectionPipe> | null;
   ...
   * @return new array instance with this as only element.
   */
  singletonSet(): Set_Type<YoloSymbolDetectionPipe>;

  /**
   * Creates a set of instances of this type.
   */
  static setBuilder(): SetBuilder<YoloSymbolDetectionPipe> | null;

  /**
   * Create a map of string to elements of this type.
   */
  static mapBuilder(): MapBuilder<string, YoloSymbolDetectionPipe> | null;

  /**
   * Create a map with the given key type and elements of this type.
   */
  static mapBuilderOf(keyType: ValueType): MapBuilder<any, YoloSymbolDetectionPipe> | null;

  /**
   * @return new ObjBuilder with initial state set to fields of this instance.
   */
  toBuilder(): ObjBuilder<YoloSymbolDetectionPipe>;

  /**
   * @return new ObjBuilder of this instance.
   */
  static builder(): ObjBuilder<YoloSymbolDetectionPipe>;

  /**
   * Construct instance of this type from provided field values and options
   */
  static fromFields(fields: Map_Type<FieldType, any>, spec: Obj.MakeSpec): YoloSymbolDetectionPipe;
   ...
   * @see withDefaults
   */
  static fromFields(fields: Map_Type<FieldType, any | any>, withDefaults?: boolean): YoloSymbolDetectionPipe;

  /**
   * Construct instance of this type from provided field values and options
   */
  static make(fields: Map_Type<string, any>, spec: Obj.MakeSpec): YoloSymbolDetectionPipe;
   ...
   * @see withDefaults
   */
  static make(withDefaults?: boolean): YoloSymbolDetectionPipe;
   ...
   * @see withDefaults
   */
  static make(fields: Map_Type<string, any | any>, withDefaults?: boolean): YoloSymbolDetectionPipe;
   ...
   * @see withDefaults
   */
  static make(fields: any, withDefaults?: boolean): YoloSymbolDetectionPipe;
   ...
   * @see #fromString
   */
  static make(s: string): YoloSymbolDetectionPipe | null;

  /**
   * Construct an instance of this type from provided instance of a subtype or a "duck type".
   */
  static remake(other: Obj, failIfExtraOrInvalidFields?: boolean): YoloSymbolDetectionPipe;
   ...
   * Note that it introduces additional overhead so should only be implemented for low volume data.
   */
  afterMake(): YoloSymbolDetectionPipe;
   ...
   */
  static cachedEmptyInst(): YoloSymbolDetectionPipe;

  /**
   * Generate a stream of instances of this type. The stream is endless and will call #generateObj each time a new
   * value is read.
   */
  static generateObjs(spec?: Obj.GenerateSpec): Stream<YoloSymbolDetectionPipe> | null;
   ...
   */
  static generateObj(spec?: Obj.GenerateSpec): YoloSymbolDetectionPipe;
   ...
   */
  remakeTemplate(): YoloSymbolDetectionPipe;
   ...
   */
  withoutIdentity(): YoloSymbolDetectionPipe;
   ...
   */
  trainSync(x: Data, y?: , spec?: MlOperationSpec): YoloSymbolDetectionPipe;
   ...
   */
  static allCached(doNotProduceAll?: boolean): Stream<YoloSymbolDetectionPipe> | null;
   ...
   */
  static find(filter?: string, doNotProduceAll?: boolean): Stream<YoloSymbolDetectionPipe> | null;
   ...
   * @return existing instances of this type that satisfy filter from data cache.
   */
  static findBy(field: string, value: any, doNotProduceAll?: boolean): Stream<YoloSymbolDetectionPipe> | null;
   ...
   * @see getCached
   */
  static findByCacheKey(key: string): YoloSymbolDetectionPipe | null;
   ...
   * @see findByCacheKey
   */
  static forCacheKey(key: string): YoloSymbolDetectionPipe | null;
   ...
   * @see findByCacheKey
   */
  getCached(): YoloSymbolDetectionPipe | null;

  /**
   * @return newly produced cached instance.
   */
  refreshCache(): YoloSymbolDetectionPipe | null;
   ...
   */
  static produce(key: string): YoloSymbolDetectionPipe | null;
   ...
   */
  static produceAll(): Map_Type<string, YoloSymbolDetectionPipe> | null;

  /**
   * Override for sub-types to produce all values.
   */
  static doProduceAll(): Map_Type<string, YoloSymbolDetectionPipe> | null;
   ...
   */
  static fetch(spec?: FetchSpec): FetchResult<YoloSymbolDetectionPipe>;
   ...
   * @return Requested objs.
   */
  static fetch(filter: Filter): FetchResult<YoloSymbolDetectionPipe>;
   ...
   */
  static fetchObjStream(spec?: FetchStreamSpec): Stream<YoloSymbolDetectionPipe> | null;
   ...
   */
  static fetchMultiLocale(spec?: MultiLocaleFetchSpec): Map_Type<string, FetchResult<YoloSymbolDetectionPipe>> | null;
   ...
   */
  get(include?: string): YoloSymbolDetectionPipe | null;
   ...
   *         those fields will be returned. Otherwise the entire obj will be returned.
   */
  getSpecific(include?: string): YoloSymbolDetectionPipe | null;
   ...
   */
  getMissing(spec: GetMissingSpec): YoloSymbolDetectionPipe;
   ...
   *         those fields will be returned. Otherwise the entire obj will be returned.
   */
  getDirect(include?: string): YoloSymbolDetectionPipe | null;
   ...
   */
  applyReverseEdit(versionEdit: VersionEdit): YoloSymbolDetectionPipe;
   ...
   * @see #toString
   */
  static fromString(s: string): YoloSymbolDetectionPipe | null;
   ...
   */
  create(spec?: UpsertSpec): YoloSymbolDetectionPipe | null;
   ...
   */
  static createBatch(objs: Array_Type<YoloSymbolDetectionPipe>, spec?: UpsertSpec): ObjList<YoloSymbolDetectionPipe> | null;
   ...
   */
  static createBatchObjStream(objs: Stream<YoloSymbolDetectionPipe>, spec?: CreateBatchObjStreamSpec): ObjList<YoloSymbolDetectionPipe> | null;
   ...
   */
  update(srcObj?: YoloSymbolDetectionPipe, spec?: UpsertSpec): YoloSymbolDetectionPipe | null;
   ...
   */
  upsert(srcObj?: YoloSymbolDetectionPipe, spec?: UpsertSpec): YoloSymbolDetectionPipe | null;
   ...
   */
  merge(spec?: MergeSpec): YoloSymbolDetectionPipe | null;
   ...
   * @return The created or updated obj.
   */
  merge(mergeInclude: string, spec?: MergeSpec): YoloSymbolDetectionPipe | null;
   ...
   * @return The created or updated obj.
   */
  touch(spec?: TouchSpec): YoloSymbolDetectionPipe | null;
   ...
   */
  static updateBatch(objs: Array_Type<YoloSymbolDetectionPipe>, srcObjs?: Array_Type<YoloSymbolDetectionPipe>, spec?: UpsertSpec): ObjList<YoloSymbolDetectionPipe> | null;
   ...
   *         populated. Otherwise only the id field will be populated.
   */
  static updateObjs(objs: λSupplier<Array_Type<YoloSymbolDetectionPipe> | null>, spec?: UpsertSpec): ObjList<YoloSymbolDetectionPipe> | null;
   ...
   *         fields populated. Otherwise only the id field will be populated.
   */
  static upsertBatch(objs: Array_Type<YoloSymbolDetectionPipe>, srcObjs?: Array_Type<YoloSymbolDetectionPipe>, spec?: UpsertSpec): ObjList<YoloSymbolDetectionPipe> | null;
   ...
   *         fields populated. Otherwise only the id field will be populated.
   */
  static upsertObjs(objs: λSupplier<Array_Type<YoloSymbolDetectionPipe> | null>, spec?: UpsertSpec): ObjList<YoloSymbolDetectionPipe> | null;
   ...
   *         populated. Otherwise only the id field will be populated.
   */
  static touchBatch(objs: Array_Type<YoloSymbolDetectionPipe>, spec?: TouchSpec): ObjList<YoloSymbolDetectionPipe> | null;
   ...
   * @see Mergespec#csvInclude
   */
  static mergeBatch(objs: Array_Type<YoloSymbolDetectionPipe>, spec?: MergeSpec): ObjList<YoloSymbolDetectionPipe> | null;
   ...
   * @see Mergespec#csvInclude
   */
  static mergeBatch(objs: Array_Type<YoloSymbolDetectionPipe>, mergeInclude: string, spec?: MergeSpec): ObjList<YoloSymbolDetectionPipe> | null;
   ...
   * @see Mergespec#csvInclude
   */
  static mergeObjs(objs: λSupplier<Array_Type<YoloSymbolDetectionPipe> | null>, mergeInclude: string, spec?: MergeSpec): ObjList<YoloSymbolDetectionPipe> | null;
   ...
   */
  static mergeAll(mergeObj: YoloSymbolDetectionPipe, spec?: MergeAllSpec): number | null;
   ...
   * @return The number of objs that were merged.
   */
  static mergeAll(mergeObj: YoloSymbolDetectionPipe, mergeInclude: string, spec?: MergeAllSpec): number | null;
   ...
   * @see Ann.Db#archive
   */
  static removeBatch(objs: Array_Type<YoloSymbolDetectionPipe>, spec?: UpsertSpec): ObjList<YoloSymbolDetectionPipe> | null;
   ...
   *         populated. Otherwise only the id field will be populated.
   */
  static replace(objs: Array_Type<YoloSymbolDetectionPipe>, spec?: UpsertSpec): ObjList<YoloSymbolDetectionPipe> | null;
   ...
   * @see Ann.Db#archive
   */
  unremove(): YoloSymbolDetectionPipe | null;
   ...
   */
  static beforeCreate(objs: Array_Type<YoloSymbolDetectionPipe>): ObjList<YoloSymbolDetectionPipe>;
   ...
   */
  static beforeUpdate(objs: Array_Type<YoloSymbolDetectionPipe>): ObjList<YoloSymbolDetectionPipe>;
   ...
   *         of objs must match the input.
   */
  static beforeRemove(objs: Array_Type<YoloSymbolDetectionPipe>): ObjList<YoloSymbolDetectionPipe>;
   ...
   */
  static afterCreate(objs: Array_Type<YoloSymbolDetectionPipe>): Array_Type<ObjError> | null;
   ...
   * @return List of any errors that were encountered.
   */
  static afterUpdate(objs: Array_Type<YoloSymbolDetectionPipe>): Array_Type<ObjError> | null;
   ...
   * @return List of any errors that were encountered.
   */
  static afterRemove(objs: Array_Type<YoloSymbolDetectionPipe>): Array_Type<ObjError> | null;
   ...
   */
  static startImportData(spec?: StartImportDataSpec): PushStream<YoloSymbolDetectionPipe>;

  /**
   * @return a push stream to import instances of this type.
   */
  static startImportDataWithStats(spec?: StartImportDataSpec): PushStreamWithStats<YoloSymbolDetectionPipe>;
   ...
   */
  static forId(id: string, failIfMissing?: boolean): YoloSymbolDetectionPipe | null;
   ...
   */
  static typeOfBatch(objs: Array_Type<YoloSymbolDetectionPipe>): Array_Type<EntityType> | null;
   ...
   */
  static dbEcho(template?: YoloSymbolDetectionPipe, count?: number, sendBack?: boolean): number | null;
   ...
   */
  static callbackLogic(objs: Array_Type<YoloSymbolDetectionPipe>, callback?: λFunction<YoloSymbolDetectionPipe | null, YoloSymbolDetectionPipe | null>): ObjList<YoloSymbolDetectionPipe>;

  /**
   * Implements a simple logic for Persistable "after" call-backs like after create by looping each input obj and
   * calling a transform for it.
   */
  static afterCallbackLogic(objs: Array_Type<YoloSymbolDetectionPipe>, callback?: λFunction<YoloSymbolDetectionPipe | null, YoloSymbolDetectionPipe | null>): Array_Type<ObjError> | null;
   ...
   */
  publish(subPath?: string, contentType?: string): YoloSymbolDetectionPipe;
   ...
   */
  static removeSeedDataBatch(objs?: Array_Type<SeedData>, spec?: UpsertSpec): ObjList<YoloSymbolDetectionPipe> | null;
   ...
   */
  createSeedData(spec?: UpsertSpec): YoloSymbolDetectionPipe | null;
   ...
   */
  static createSeedDataBatch(objs?: Array_Type<SeedData>, spec?: UpsertSpec): ObjList<YoloSymbolDetectionPipe> | null;
   ...
   */
  updateSeedData(srcObj?: SeedData, spec?: UpsertSpec): YoloSymbolDetectionPipe | null;
   ...
   */
  static updateSeedDataBatch(objs?: Array_Type<SeedData>, srcObjs?: Array_Type<SeedData>, spec?: UpsertSpec): ObjList<YoloSymbolDetectionPipe> | null;
   ...
   * @return updated obj instance, same as {@link Persistable#update} return value.
   */
  clearUserUpdates(spec?: MergeSpec): YoloSymbolDetectionPipe | null;
   ...
   *         {@link Persistable#updateBatch}.
   */
  static clearUserUpdatesBatch(objs?: Array_Type<SeedData>, spec?: MergeSpec): ObjList<YoloSymbolDetectionPipe> | null;
  ...
  dependencies(): SeedDataDeps<YoloSymbolDetectionPipe> | null;
   ...
   * be returned as-is.
   */
  byReference(): YoloSymbolDetectionPipe;
   ...
   */
  updateModelFiles(modelFiles: Map_Type<string, Url>, spec?: MlOperationSpec): YoloSymbolDetectionPipe;
   ...
   */
  afterTrain(doTrainOutputs: any, spec?: MlOperationSpec): YoloSymbolDetectionPipe;
   ...
   */
  withNewSpec(spec?: MlConvertSpec): YoloSymbolDetectionPipe;
   ...
   */
  afterConvert(model: any, spec?: MlConvertSpec): YoloSymbolDetectionPipe;
   ...
   */
  withHyperparamSpaces(paramSpaces: Map_Type<string, Hp.ParamSpace>): YoloSymbolDetectionPipe;
