...
declare module 'c3-thin' {
  ...
  export class ContentValue {
    ...
    static fromBinary(value?: ArrayBuffer, contentType?: string, contentEncoding?: string): ContentValue;

    static fromString(value?: string, contentType?: string): ContentValue;
  }

  export class AsyncThinTypeSystem {
    get: (name: string, failIfMissing?: string) => Promise<AsyncTypeProxy>;
  }

  export class AsyncTypeProxy extends TypeProxyBase {
    constructor(ts: AsyncThinTypeSystem, name?: string);

    call(action: string, args?: { [arg: string]: any }): Promise<any>;
  ...
  export class ServerConnection {
    ...
    asyncThinTypeSys(): AsyncThinTypeSystem;
  }

  export class AsyncServerRequest extends ServerRequestBase {
    send(content?: string | ArrayBuffer | Uint8Array | Record<string, unknown>): Promise<ServerResponse>;
  }

  export class ServerRequest extends ServerRequestBase {
    constructor(conn: ServerConnection, init?: ServerRequestInit);

    send(content?: string | ArrayBuffer | Uint8Array | Record<string, unknown>): ServerResponse;
  }
