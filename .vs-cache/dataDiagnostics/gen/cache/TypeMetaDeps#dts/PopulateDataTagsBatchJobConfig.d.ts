// TypeScript definitions for the C3 type PopulateDataTagsBatchJobConfig

/**
 * Specifies the configuration of the {@link PopulateDataTagsBatchJob} such as how it wil schedule batches for {@link DataTagBase}
 * instances which each scan the entirety of a table for a {@link DataTagBase#srcType}.
 *
 * @remarks this represents a value passed to a method that expects an instance of PopulateDataTagsBatchJobConfig
 */
declare interface IPopulateDataTagsBatchJobConfig {
  ...
  /**
   * Whether or not to batch all of the {@link DataTagBase} instances that evaluate expressions on the same
   * {@link DataTag#srcType} and have a {@link DataTagBase#filter} which scans the whole table into the same batches.
   */
  batchAllScanningExpressions?: boolean;

  /**
   * The size of the batches in terms of the number of instances of {@link DataTagBase#srcType} should be placed into
   * each batch of the {@link PopulateDataTagsBatchJob}
   */
  scanningExpressionsBatchSize?: number;

  /**
   * Whether or not {@link DataTagCountHistory}s should be persisted for each data tag after the {@link PopulateDataTagsBatchJob}
   * completes.
   */
  persistTagCountsAfterJob?: boolean;

  /**
   * Whether or not data tags with {@link DataTagBase#enabled} set to true should be the only tags which have counts
   * persisted.
   */
  persistEnabledTagCountsOnly?: boolean;

  /**
   * Whether or not the persistence of the {@link DataTagCountHistory} instances should happen asynchronously for
   * each {@link DataTagBase#srcType} being tagged.
   */
  persistTagCountsAsynchronously?: boolean;
}
 ...
 * instances which each scan the entirety of a table for a {@link DataTagBase#srcType}.
 *
 * @remarks this represents a made instance of PopulateDataTagsBatchJobConfig
 */
declare class PopulateDataTagsBatchJobConfig extends Obj  {

  readonly configOverride?: string;
  withConfigOverride(configOverride: string | null): PopulateDataTagsBatchJobConfig;

  static readonly configOverride?: string;

  readonly secretOverride?: string;
  withSecretOverride(secretOverride: string | null): PopulateDataTagsBatchJobConfig;
  ...
  withIssues(issues: Array_Type<string> | Array<string> | null): PopulateDataTagsBatchJobConfig;
   ...
   * {@link DataTag#srcType} and have a {@link DataTagBase#filter} which scans the whole table into the same batches.
   */
  readonly batchAllScanningExpressions?: boolean;
  withBatchAllScanningExpressions(batchAllScanningExpressions: boolean): PopulateDataTagsBatchJobConfig;

  /**
   * Whether or not to batch all of the {@link DataTagBase} instances that evaluate expressions on the same
   * {@link DataTag#srcType} and have a {@link DataTagBase#filter} which scans the whole table into the same batches.
   */
  static readonly batchAllScanningExpressions?: boolean;

  /**
   * The size of the batches in terms of the number of instances of {@link DataTagBase#srcType} should be placed into
   * each batch of the {@link PopulateDataTagsBatchJob}
   */
  readonly scanningExpressionsBatchSize?: number;
  withScanningExpressionsBatchSize(scanningExpressionsBatchSize: number | null): PopulateDataTagsBatchJobConfig;

  /**
   * The size of the batches in terms of the number of instances of {@link DataTagBase#srcType} should be placed into
   * each batch of the {@link PopulateDataTagsBatchJob}
   */
  static readonly scanningExpressionsBatchSize?: number;

  /**
   * Whether or not {@link DataTagCountHistory}s should be persisted for each data tag after the {@link PopulateDataTagsBatchJob}
   * completes.
   */
  readonly persistTagCountsAfterJob?: boolean;
  withPersistTagCountsAfterJob(persistTagCountsAfterJob: boolean): PopulateDataTagsBatchJobConfig;

  /**
   * Whether or not {@link DataTagCountHistory}s should be persisted for each data tag after the {@link PopulateDataTagsBatchJob}
   * completes.
   */
  static readonly persistTagCountsAfterJob?: boolean;

  /**
   * Whether or not data tags with {@link DataTagBase#enabled} set to true should be the only tags which have counts
   * persisted.
   */
  readonly persistEnabledTagCountsOnly?: boolean;
  withPersistEnabledTagCountsOnly(persistEnabledTagCountsOnly: boolean): PopulateDataTagsBatchJobConfig;

  /**
   * Whether or not data tags with {@link DataTagBase#enabled} set to true should be the only tags which have counts
   * persisted.
   */
  static readonly persistEnabledTagCountsOnly?: boolean;

  /**
   * Whether or not the persistence of the {@link DataTagCountHistory} instances should happen asynchronously for
   * each {@link DataTagBase#srcType} being tagged.
   */
  readonly persistTagCountsAsynchronously?: boolean;
  withPersistTagCountsAsynchronously(persistTagCountsAsynchronously: boolean): PopulateDataTagsBatchJobConfig;

  /**
   * Whether or not the persistence of the {@link DataTagCountHistory} instances should happen asynchronously for
   * each {@link DataTagBase#srcType} being tagged.
   */
  static readonly persistTagCountsAsynchronously?: boolean;
   ...
   * @see #toJson
   */
  static fromJson(json: any): PopulateDataTagsBatchJobConfig | null;
   ...
   * @see #toJsonString
   */
  static fromJsonString(json: string): PopulateDataTagsBatchJobConfig | null;
   ...
   * @see #toXmlString
   */
  static fromXmlString(xml: string): PopulateDataTagsBatchJobConfig | null;
   ...
   */
  static deserialize(contentStr: string, contentType: string): PopulateDataTagsBatchJobConfig | null;
   ...
   */
  replaceType(old: Type, new_: Type): PopulateDataTagsBatchJobConfig;
   ...
   * This method is used during live metadata update
   */
  static replaceType(old: Type, new_: Type): PopulateDataTagsBatchJobConfig;
   ...
   */
  mapFieldValues(mapper: λBiFunction<FieldType, any, any | null>, convertValue?: boolean): PopulateDataTagsBatchJobConfig;
   ...
   *           if true, attempt to convert the value to match the field's type
   */
  static mapFieldValues(mapper: λBiFunction<FieldType, any, any | null>, convertValue?: boolean): PopulateDataTagsBatchJobConfig;
   ...
   *           if true, attempt to convert the value to match the field's type
   */
  mapFieldValues(spec: ValueSpec, mapper: λBiFunction<FieldType, any, any | null>, convertValue?: boolean): PopulateDataTagsBatchJobConfig;
   ...
   *           if true, attempt to convert the value to match the field's type
   */
  static mapFieldValues(spec: ValueSpec, mapper: λBiFunction<FieldType, any, any | null>, convertValue?: boolean): PopulateDataTagsBatchJobConfig;
   ...
   *           if true, attempt to convert the value to match the field's type
   */
  mapFieldValuesAsync(mapper: λBiFunction<FieldType, any, Promise<any> | null>, convertValue?: boolean): Promise<PopulateDataTagsBatchJobConfig>;
   ...
   *           if true, attempt to convert the value to match the field's type
   */
  static mapFieldValuesAsync(mapper: λBiFunction<FieldType, any, Promise<any> | null>, convertValue?: boolean): Promise<PopulateDataTagsBatchJobConfig>;
   ...
   *           if true, attempt to convert the value to match the field's type
   */
  mapFieldValuesAsync(spec: ValueSpec, mapper: λBiFunction<FieldType | null, any, Promise<any> | null>, convertValue?: boolean): Promise<PopulateDataTagsBatchJobConfig>;
   ...
   *           if true, attempt to convert the value to match the field's type
   */
  static mapFieldValuesAsync(spec: ValueSpec, mapper: λBiFunction<FieldType | null, any, Promise<any> | null>, convertValue?: boolean): Promise<PopulateDataTagsBatchJobConfig>;
   ...
   *           if true, attempt to convert the value to match the field's type
   */
  mapFieldValue(field?: FieldType, includeEmpty?: boolean, mapper: λFunction<any, any | null>, convertValue?: boolean): PopulateDataTagsBatchJobConfig;
   ...
   *           if true, attempt to convert the value to match the field's type
   */
  static mapFieldValue(field?: FieldType, includeEmpty?: boolean, mapper: λFunction<any, any | null>, convertValue?: boolean): PopulateDataTagsBatchJobConfig;
   ...
   *           if true, attempt to convert the value to match the field's type
   */
  mapRefs(mapper: λBiFunction<FieldType, Obj, Obj | null>, convertValue?: boolean): PopulateDataTagsBatchJobConfig;
   ...
   *           if true, attempt to convert the value to match the field's type
   */
  static mapRefs(mapper: λBiFunction<FieldType, Obj, Obj | null>, convertValue?: boolean): PopulateDataTagsBatchJobConfig;
   ...
   *           if true, attempt to convert the value to match the field's type
   */
  mapRefs(includeEmpty: boolean, mapper: λBiFunction<FieldType, Obj, Obj | null>, convertValue?: boolean): PopulateDataTagsBatchJobConfig;
   ...
   *           if true, attempt to convert the value to match the field's type
   */
  static mapRefs(includeEmpty: boolean, mapper: λBiFunction<FieldType, Obj, Obj | null>, convertValue?: boolean): PopulateDataTagsBatchJobConfig;

  /**
   * Populates all missing default values and throws error if any constraint is violated.
   */
  validateObj(): PopulateDataTagsBatchJobConfig;

  /**
   * Populates all missing default values and throws error if any constraint is violated.
   */
  static validateObj(): PopulateDataTagsBatchJobConfig;
   ...
   * @see #defaultField
   */
  withField(field: string, value: any, doNotConvert?: boolean): PopulateDataTagsBatchJobConfig;
   ...
   * @see #defaultField
   */
  static withField(field: string, value: any, doNotConvert?: boolean): PopulateDataTagsBatchJobConfig;
   ...
   * @see #defaultField
   */
  withField(field: FieldType, value: any, doNotConvert?: boolean): PopulateDataTagsBatchJobConfig;
   ...
   * @see #defaultField
   */
  static withField(field: FieldType, value: any, doNotConvert?: boolean): PopulateDataTagsBatchJobConfig;
   ...
   * @return new Obj
   */
  withFields(fields: Map_Type<string, any>, doNotConvert?: boolean): PopulateDataTagsBatchJobConfig;
   ...
   * @return new Obj
   */
  static withFields(fields: Map_Type<string, any>, doNotConvert?: boolean): PopulateDataTagsBatchJobConfig;
   ...
   * @return new Obj
   */
  withFieldAtPath(path: string, value: any, doNotConvert?: boolean, doNotCreateIfMissing?: boolean): PopulateDataTagsBatchJobConfig;
   ...
   * @return new Obj
   */
  static withFieldAtPath(path: string, value: any, doNotConvert?: boolean, doNotCreateIfMissing?: boolean): PopulateDataTagsBatchJobConfig;
   ...
   * @see #withoutField
   */
  withoutFieldAtPath(path: string): PopulateDataTagsBatchJobConfig;
   ...
   * @see #withoutField
   */
  static withoutFieldAtPath(path: string): PopulateDataTagsBatchJobConfig;
   ...
   * @see #removeField
   */
  withoutField(field: string): PopulateDataTagsBatchJobConfig;
   ...
   * @see #removeField
   */
  static withoutField(field: string): PopulateDataTagsBatchJobConfig;
   ...
   * @see #removeField
   */
  withoutField(field: FieldType): PopulateDataTagsBatchJobConfig;
   ...
   * @see #removeField
   */
  static withoutField(field: FieldType): PopulateDataTagsBatchJobConfig;
   ...
   */
  withoutFields(fields: Array_Type<string>): PopulateDataTagsBatchJobConfig;
   ...
   * @return new Obj with removed fields
   */
  static withoutFields(fields: Array_Type<string>): PopulateDataTagsBatchJobConfig;
   ...
   * @return new Obj with removed fields
   */
  withoutFieldsByType(fields: Array_Type<FieldType>): PopulateDataTagsBatchJobConfig;
   ...
   * @return new Obj with removed fields
   */
  static withoutFieldsByType(fields: Array_Type<FieldType>): PopulateDataTagsBatchJobConfig;

  /**
   * @return a new Obj, removing the field types marked with annotation @config(secret=true) recursively
   */
  withoutSecretFields(): PopulateDataTagsBatchJobConfig;

  /**
   * @return a new Obj, removing the field types marked with annotation @config(secret=true) recursively
   */
  static withoutSecretFields(): PopulateDataTagsBatchJobConfig;
   ...
   */
  withDefaults(includeEmptyRefsWithDefaults?: boolean, defaultFields?: Array_Type<string>): PopulateDataTagsBatchJobConfig;
   ...
   * @see FieldType#defaultValue
   */
  static withDefaults(includeEmptyRefsWithDefaults?: boolean, defaultFields?: Array_Type<string>): PopulateDataTagsBatchJobConfig;
   ...
   * @see #unsetField
   */
  defaultField(field: string): PopulateDataTagsBatchJobConfig;
   ...
   * @see #unsetField
   */
  static defaultField(field: string): PopulateDataTagsBatchJobConfig;
   ...
   * @see #unsetField
   */
  defaultField(field: FieldType): PopulateDataTagsBatchJobConfig;
   ...
   * @see #unsetField
   */
  static defaultField(field: FieldType): PopulateDataTagsBatchJobConfig;
   ...
   * @see #removeField
   */
  unsetField(field: string): PopulateDataTagsBatchJobConfig;
   ...
   * @see #removeField
   */
  static unsetField(field: string): PopulateDataTagsBatchJobConfig;
   ...
   * @see #removeField
   */
  unsetField(field: FieldType): PopulateDataTagsBatchJobConfig;
   ...
   * @see #removeField
   */
  static unsetField(field: FieldType): PopulateDataTagsBatchJobConfig;
   ...
   * @see #unsetField
   */
  removeField(field: string): PopulateDataTagsBatchJobConfig;
   ...
   * @see #unsetField
   */
  static removeField(field: string): PopulateDataTagsBatchJobConfig;
   ...
   * @see #unsetField
   */
  removeField(field: FieldType): PopulateDataTagsBatchJobConfig;
   ...
   * @see #unsetField
   */
  static removeField(field: FieldType): PopulateDataTagsBatchJobConfig;
   ...
   */
  mergeObj(other: Obj, fieldPathMergeSpec?: Map_Type<string, string>): PopulateDataTagsBatchJobConfig;
   ...
   * @return the new merged Obj
   */
  static mergeObj(other: Obj, fieldPathMergeSpec?: Map_Type<string, string>): PopulateDataTagsBatchJobConfig;
   ...
   * @return the new merged Obj
   */
  mergeObj(other: Obj, otherFieldsFilter: Type): PopulateDataTagsBatchJobConfig;
   ...
   * @return the new merged Obj
   */
  static mergeObj(other: Obj, otherFieldsFilter: Type): PopulateDataTagsBatchJobConfig;
   ...
   */
  mergeObj(other: Obj, deep?: boolean, merger: λQuadFunction<FieldPath | null, any | null, FieldPath | null, any | null, any | null>): PopulateDataTagsBatchJobConfig;
   ...
   *        the same key or index.
   */
  static mergeObj(other: Obj, deep?: boolean, merger: λQuadFunction<FieldPath | null, any | null, FieldPath | null, any | null, any | null>): PopulateDataTagsBatchJobConfig;
   ...
   */
  mergeObj(other: Obj, merger: λQuadFunction<FieldType | null, any | null, FieldType | null, any | null, any | null>): PopulateDataTagsBatchJobConfig;
   ...
   * traverse child reference and collection fields.
   */
  static mergeObj(other: Obj, merger: λQuadFunction<FieldType | null, any | null, FieldType | null, any | null, any | null>): PopulateDataTagsBatchJobConfig;

  mergeJson(json: any): PopulateDataTagsBatchJobConfig;

  static mergeJson(json: any): PopulateDataTagsBatchJobConfig;
   ...
   */
  mergeChildren(deep?: boolean, objKey?: λFunction<Obj | null, any | null>, filter?: λPredicate<string>): PopulateDataTagsBatchJobConfig;
   ...
   * @return Obj with child references merged
   */
  static mergeChildren(deep?: boolean, objKey?: λFunction<Obj | null, any | null>, filter?: λPredicate<string>): PopulateDataTagsBatchJobConfig;
   ...
   */
  sumObj(other: Obj, deep?: boolean): PopulateDataTagsBatchJobConfig;
   ...
   * references with same name and collection elements at same index or key.
   */
  static sumObj(other: Obj, deep?: boolean): PopulateDataTagsBatchJobConfig;
   ...
   */
  singletonArray(): Array_Type<PopulateDataTagsBatchJobConfig>;
   ...
   * @return new array instance with this as only element.
   */
  static singletonArray(): Array_Type<PopulateDataTagsBatchJobConfig>;

  /**
   * Creates an array of instances of this type.
   */
  static array(...elements: Array_Type<any>[]): Array_Type<PopulateDataTagsBatchJobConfig> | null;

  /**
   * Creates an array of instances of this type.
   */
  static arrayBuilder(): ArrayBuilder<PopulateDataTagsBatchJobConfig> | null;
   ...
   * @return new array instance with this as only element.
   */
  singletonSet(): Set_Type<PopulateDataTagsBatchJobConfig>;
   ...
   * @return new array instance with this as only element.
   */
  static singletonSet(): Set_Type<PopulateDataTagsBatchJobConfig>;

  /**
   * Creates a set of instances of this type.
   */
  static setBuilder(): SetBuilder<PopulateDataTagsBatchJobConfig> | null;

  /**
   * Create a map of string to elements of this type.
   */
  static mapBuilder(): MapBuilder<string, PopulateDataTagsBatchJobConfig> | null;

  /**
   * Create a map with the given key type and elements of this type.
   */
  static mapBuilderOf(keyType: ValueType): MapBuilder<any, PopulateDataTagsBatchJobConfig> | null;

  /**
   * @return new ObjBuilder with initial state set to fields of this instance.
   */
  toBuilder(): ObjBuilder<PopulateDataTagsBatchJobConfig>;

  /**
   * @return new ObjBuilder with initial state set to fields of this instance.
   */
  static toBuilder(): ObjBuilder<PopulateDataTagsBatchJobConfig>;

  /**
   * @return new ObjBuilder of this instance.
   */
  static builder(): ObjBuilder<PopulateDataTagsBatchJobConfig>;

  /**
   * Construct instance of this type from provided field values and options
   */
  static fromFields(fields: Map_Type<FieldType, any>, spec: Obj.MakeSpec): PopulateDataTagsBatchJobConfig;
   ...
   * @see withDefaults
   */
  static fromFields(fields: Map_Type<FieldType, any | any>, withDefaults?: boolean): PopulateDataTagsBatchJobConfig;

  /**
   * Construct instance of this type from provided field values and options
   */
  static make(fields: Map_Type<string, any>, spec: Obj.MakeSpec): PopulateDataTagsBatchJobConfig;
   ...
   * @see withDefaults
   */
  static make(withDefaults?: boolean): PopulateDataTagsBatchJobConfig;
   ...
   * @see withDefaults
   */
  static make(fields: Map_Type<string, any | any>, withDefaults?: boolean): PopulateDataTagsBatchJobConfig;
   ...
   * @see withDefaults
   */
  static make(fields: any, withDefaults?: boolean): PopulateDataTagsBatchJobConfig;

  /**
   * Construct an instance of this type from provided instance of a subtype or a "duck type".
   */
  static remake(other: Obj, failIfExtraOrInvalidFields?: boolean): PopulateDataTagsBatchJobConfig;
   ...
   * Note that it introduces additional overhead so should only be implemented for low volume data.
   */
  afterMake(): PopulateDataTagsBatchJobConfig;
   ...
   * Note that it introduces additional overhead so should only be implemented for low volume data.
   */
  static afterMake(): PopulateDataTagsBatchJobConfig;
   ...
   */
  static cachedEmptyInst(): PopulateDataTagsBatchJobConfig;

  /**
   * Generate a stream of instances of this type. The stream is endless and will call #generateObj each time a new
   * value is read.
   */
  static generateObjs(spec?: Obj.GenerateSpec): Stream<PopulateDataTagsBatchJobConfig> | null;
   ...
   */
  static generateObj(spec?: Obj.GenerateSpec): PopulateDataTagsBatchJobConfig;
   ...
   */
  static allCached(): Stream<PopulateDataTagsBatchJobConfig> | null;
   ...
   */
  static find(filter?: string): Stream<PopulateDataTagsBatchJobConfig> | null;
   ...
   *         Otherwise no population will be performed.
   */
  static findBy(field: string, value: any): Stream<PopulateDataTagsBatchJobConfig> | null;

  /**
   * @return cached instance.
   */
  getCached(): PopulateDataTagsBatchJobConfig | null;

  /**
   * @return cached instance.
   */
  static getCached(): PopulateDataTagsBatchJobConfig | null;

  /**
   * Returns cached instance by key, with secrets removed.
   */
  static forCacheKey(key: string): PopulateDataTagsBatchJobConfig | null;
   ...
   */
  static produce(key: string): PopulateDataTagsBatchJobConfig | null;

  /**
   * Should produces the entire content for the cached data. Will populate the cache with the produced data.
   */
  static produceAll(): Map_Type<string, PopulateDataTagsBatchJobConfig> | null;
   ...
   */
  getConfig(): PopulateDataTagsBatchJobConfig;
   ...
   * Note that typical pattern is to use Configurable#config instead of this method.
   */
  static getConfig(): PopulateDataTagsBatchJobConfig;
   ...
   */
  getSecret(): PopulateDataTagsBatchJobConfig;
   ...
   * Note that typical pattern is to use Configurable#configWithSecrets instead of this method.
   */
  static getSecret(): PopulateDataTagsBatchJobConfig;
   ...
   */
  withConfigKey(configKey: string): PopulateDataTagsBatchJobConfig;
   ...
   *     config.configKey() => "folder1/abc"
   */
  static withConfigKey(configKey: string): PopulateDataTagsBatchJobConfig;

  /**
   * @return copy without config key.
   */
  withoutConfigKey(): PopulateDataTagsBatchJobConfig;

  /**
   * @return copy without config key.
   */
  static withoutConfigKey(): PopulateDataTagsBatchJobConfig;

  /**
   * @return return cached config instance for provided config key.
   */
  static forConfigKey(configKey: string): PopulateDataTagsBatchJobConfig | null;
   ...
   */
  static listConfigs(parallel?: boolean, filter?: λPredicate<string>): Stream<PopulateDataTagsBatchJobConfig> | null;
   ...
   * @see setConfigValue
   */
  setConfigValues(from: PopulateDataTagsBatchJobConfig, override?: string): void;
   ...
   * @see setConfigValue
   */
  static setConfigValues(from: PopulateDataTagsBatchJobConfig, override?: string): void;
   ...
   */
  static loadConfigAndSecret(configKey: string): PopulateDataTagsBatchJobConfig | null;
   ...
   */
  removeSecrets(): PopulateDataTagsBatchJobConfig | null;

  /**
   * @return this config without any secret values.
   */
  static removeSecrets(): PopulateDataTagsBatchJobConfig | null;

  /**
   * @return this config with only secret values.
   */
  removeNonSecrets(): PopulateDataTagsBatchJobConfig | null;

  /**
   * @return this config with only secret values.
   */
  static removeNonSecrets(): PopulateDataTagsBatchJobConfig | null;

  /**
   * Validates and alters fields in the configuration to fit specific requirements. sub-types can override to add custom
   * validation and setup.
   */
  validate(): PopulateDataTagsBatchJobConfig;

  /**
   * Validates and alters fields in the configuration to fit specific requirements. sub-types can override to add custom
   * validation and setup.
   */
  static validate(): PopulateDataTagsBatchJobConfig;
   ...
   */
  static inst(): PopulateDataTagsBatchJobConfig;
}
