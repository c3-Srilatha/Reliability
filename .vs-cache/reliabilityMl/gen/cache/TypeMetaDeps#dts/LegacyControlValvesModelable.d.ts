// TypeScript definitions for the C3 type LegacyControlValvesModelable

/**
 * `LegacyControlValvesModelable` extends `ControlValvesModelableBase` and provides a model framework for reconstructing control valve data using machine learning models such as LightGBM.
 *
 * **Overview:**
 * - This entity is used in the context of modeling control valve systems.
 * - It supports reconstruction and inference processes, providing a robust way to model control valve data while considering feature weights, data completeness, and model sparsification.
 * - It integrates various machine learning techniques and pipelines for reconstructing control valve sensor data using LightGBM and ElasticNet regressors.
 *
 * **Attributes:**
 *
 * @remarks this represents a value passed to a method that expects an instance of LegacyControlValvesModelable
 */
declare interface ILegacyControlValvesModelable {
  ...
  /**
   * The {@link Feature} that represents the PERCENT_OPEN sensor instrumented on the Control Valve.
   */
  percentOpen: string;

  /**
   * Use this field to specify the list of columns in the input data that should be excluded from model training.
   *
   * If any of the values specified in this list are not columns in the input data, the pipe will ignore them, and
   * will proceed with training and inference with the remaining features.
   */
  excludeFeatures?: Array_Type<string> | Array<string>;

  /**
   * Flag to determine whether the model features will be sparsified based on the `featureWeights`.
   * This is applicable only when using an ElasticNet model. If the flag is set to `true`, the model will check the regularized coefficients of features.
   * Features with weights below the `minCoef` threshold will be excluded, and a new model will be trained without those features.
   *
   * **Note**: This option allows for feature selection based on the importance of each feature.
   */
  doNotSparsify?: boolean;

  /**
   * Minimum coefficient size for significant features when using ElasticNet regression.
   * If the absolute value of the feature weight is smaller than `minCoef`, the feature will be considered insignificant and excluded.
   * This parameter ensures that only the most relevant features are considered in the model.
   */
  minCoef?: number;

  /**
   * A dictionary that stores the feature weights for the model after training.
   * These weights represent the influence of each feature in the model’s prediction.
   * The dictionary is automatically populated during model training and should not be manually set.
   * The weights depend on the type of regressor used (e.g., ElasticNet, LightGBM) and can be used for feature importance analysis.
   */
  featureWeights?: Map_Type<string, number> | {[key: string]: number};

  /**
   * Flag to enable or disable data completeness checking during inference.
   * If set to `true`, the model will evaluate whether the available data for each feature meets a predefined completeness threshold.
   * Missing data in features will be assessed, and weighted scores will be used to determine whether the data for each feature is complete enough for accurate prediction.
   *
   * If data completeness is insufficient, no prediction will be made.
   */
  checkDataCompleteness?: boolean;

  /**
   * Minimum data completeness percentage required for making a prediction.
   * If the data completeness (based on feature weights) is below this threshold, the system will reject the prediction and may return an error or warning.
   *
   * The completeness is measured based on the weighted importance of the features used for modeling. For example, a high importance feature with insufficient data might cause the system to discard the prediction.
   */
  dataCompletenessThreshold?: number;

  /**
   * Maximum allowable duration (in hours) of missing data before the feature is considered to have insufficient data.
   * This setting is used during inference to determine when the data for a feature is considered too sparse for reliable predictions.
   *
   * For example, if a sensor has missing data for 12 consecutive hours, it will be flagged as missing, and its data will be excluded from the prediction process.
   */
  missingFeatureMaxDuration?: number;
}
 ...
 * **Attributes:**
 *
 * @remarks this represents a made instance of LegacyControlValvesModelable
 */
declare class LegacyControlValvesModelable extends Obj  {
  ...
  withTypeIdent(typeIdent: string | null): LegacyControlValvesModelable;
   ...
   */
  readonly id: string;
  withId(id: string): LegacyControlValvesModelable;
  ...
  withVersionEdits(versionEdits: Array_Type<VersionEdit> | Array<IVersionEdit> | null): LegacyControlValvesModelable;

  /**
   * Name of the Obj instance
   */
  readonly name?: string;
  withName(name: string | null): LegacyControlValvesModelable;

  /**
   * Various system fields.
   */
  readonly meta?: Meta;
  withMeta(meta: IMeta | null): LegacyControlValvesModelable;
  ...
  withVersion(version: number | null): LegacyControlValvesModelable;

  /**
   * Persists concrete type with bindings for generic types where instance has parameter bindings
   */
  readonly typeWithBindings?: Type;
  withTypeWithBindings(typeWithBindings: IType | null): LegacyControlValvesModelable;

  /**
   * The {@link Feature} that represents the PERCENT_OPEN sensor instrumented on the Control Valve.
   */
  readonly percentOpen: string;
  withPercentOpen(percentOpen: string): LegacyControlValvesModelable;
   ...
   * will proceed with training and inference with the remaining features.
   */
  readonly excludeFeatures?: Array_Type<string>;
  withExcludeFeatures(excludeFeatures: Array_Type<string> | Array<string> | null): LegacyControlValvesModelable;
   ...
   * **Note**: This option allows for feature selection based on the importance of each feature.
   */
  readonly doNotSparsify?: boolean;
  withDoNotSparsify(doNotSparsify: boolean): LegacyControlValvesModelable;
   ...
   * This parameter ensures that only the most relevant features are considered in the model.
   */
  readonly minCoef?: number;
  withMinCoef(minCoef: number | null): LegacyControlValvesModelable;
   ...
   * The weights depend on the type of regressor used (e.g., ElasticNet, LightGBM) and can be used for feature importance analysis.
   */
  readonly featureWeights?: Map_Type<string, number>;
  withFeatureWeights(featureWeights: Map_Type<string, number> | {[key: string]: number} | null): LegacyControlValvesModelable;
   ...
   * If data completeness is insufficient, no prediction will be made.
   */
  readonly checkDataCompleteness?: boolean;
  withCheckDataCompleteness(checkDataCompleteness: boolean): LegacyControlValvesModelable;
   ...
   * The completeness is measured based on the weighted importance of the features used for modeling. For example, a high importance feature with insufficient data might cause the system to discard the prediction.
   */
  readonly dataCompletenessThreshold?: number;
  withDataCompletenessThreshold(dataCompletenessThreshold: number | null): LegacyControlValvesModelable;
   ...
   * For example, if a sensor has missing data for 12 consecutive hours, it will be flagged as missing, and its data will be excluded from the prediction process.
   */
  readonly missingFeatureMaxDuration?: number;
  withMissingFeatureMaxDuration(missingFeatureMaxDuration: number | null): LegacyControlValvesModelable;
   ...
   * @see #toJson
   */
  static fromJson(json: any): LegacyControlValvesModelable | null;
   ...
   * @see #toJsonString
   */
  static fromJsonString(json: string): LegacyControlValvesModelable | null;
   ...
   * @see #toXmlString
   */
  static fromXmlString(xml: string): LegacyControlValvesModelable | null;
   ...
   */
  static deserialize(contentStr: string, contentType: string): LegacyControlValvesModelable | null;
   ...
   */
  replaceType(old: Type, new_: Type): LegacyControlValvesModelable;
   ...
   */
  mapFieldValues(mapper: λBiFunction<FieldType, any, any | null>, convertValue?: boolean): LegacyControlValvesModelable;
   ...
   *           if true, attempt to convert the value to match the field's type
   */
  mapFieldValues(spec: ValueSpec, mapper: λBiFunction<FieldType, any, any | null>, convertValue?: boolean): LegacyControlValvesModelable;
   ...
   *           if true, attempt to convert the value to match the field's type
   */
  mapFieldValuesAsync(mapper: λBiFunction<FieldType, any, Promise<any> | null>, convertValue?: boolean): Promise<LegacyControlValvesModelable>;
   ...
   *           if true, attempt to convert the value to match the field's type
   */
  mapFieldValuesAsync(spec: ValueSpec, mapper: λBiFunction<FieldType | null, any, Promise<any> | null>, convertValue?: boolean): Promise<LegacyControlValvesModelable>;
   ...
   *           if true, attempt to convert the value to match the field's type
   */
  mapFieldValue(field?: FieldType, includeEmpty?: boolean, mapper: λFunction<any, any | null>, convertValue?: boolean): LegacyControlValvesModelable;
   ...
   *           if true, attempt to convert the value to match the field's type
   */
  mapRefs(mapper: λBiFunction<FieldType, Obj, Obj | null>, convertValue?: boolean): LegacyControlValvesModelable;
   ...
   *           if true, attempt to convert the value to match the field's type
   */
  mapRefs(includeEmpty: boolean, mapper: λBiFunction<FieldType, Obj, Obj | null>, convertValue?: boolean): LegacyControlValvesModelable;

  /**
   * Populates all missing default values and throws error if any constraint is violated.
   */
  validateObj(): LegacyControlValvesModelable;
   ...
   * @see #defaultField
   */
  withField(field: string, value: any, doNotConvert?: boolean): LegacyControlValvesModelable;
   ...
   * @see #defaultField
   */
  withField(field: FieldType, value: any, doNotConvert?: boolean): LegacyControlValvesModelable;
   ...
   * @return new Obj
   */
  withFields(fields: Map_Type<string, any>, doNotConvert?: boolean): LegacyControlValvesModelable;
   ...
   * @return new Obj
   */
  withFieldAtPath(path: string, value: any, doNotConvert?: boolean, doNotCreateIfMissing?: boolean): LegacyControlValvesModelable;
   ...
   * @see #withoutField
   */
  withoutFieldAtPath(path: string): LegacyControlValvesModelable;
   ...
   * @see #removeField
   */
  withoutField(field: string): LegacyControlValvesModelable;
   ...
   * @see #removeField
   */
  withoutField(field: FieldType): LegacyControlValvesModelable;
   ...
   */
  withoutFields(fields: Array_Type<string>): LegacyControlValvesModelable;
   ...
   * @return new Obj with removed fields
   */
  withoutFieldsByType(fields: Array_Type<FieldType>): LegacyControlValvesModelable;

  /**
   * @return a new Obj, removing the field types marked with annotation @config(secret=true) recursively
   */
  withoutSecretFields(): LegacyControlValvesModelable;
   ...
   */
  withDefaults(includeEmptyRefsWithDefaults?: boolean, defaultFields?: Array_Type<string>): LegacyControlValvesModelable;
   ...
   * @see #unsetField
   */
  defaultField(field: string): LegacyControlValvesModelable;
   ...
   * @see #unsetField
   */
  defaultField(field: FieldType): LegacyControlValvesModelable;
   ...
   * @see #removeField
   */
  unsetField(field: string): LegacyControlValvesModelable;
   ...
   * @see #removeField
   */
  unsetField(field: FieldType): LegacyControlValvesModelable;
   ...
   * @see #unsetField
   */
  removeField(field: string): LegacyControlValvesModelable;
   ...
   * @see #unsetField
   */
  removeField(field: FieldType): LegacyControlValvesModelable;
   ...
   */
  mergeObj(other: Obj, fieldPathMergeSpec?: Map_Type<string, string>): LegacyControlValvesModelable;
   ...
   * @return the new merged Obj
   */
  mergeObj(other: Obj, otherFieldsFilter: Type): LegacyControlValvesModelable;
   ...
   */
  mergeObj(other: Obj, deep?: boolean, merger: λQuadFunction<FieldPath | null, any | null, FieldPath | null, any | null, any | null>): LegacyControlValvesModelable;
   ...
   */
  mergeObj(other: Obj, merger: λQuadFunction<FieldType | null, any | null, FieldType | null, any | null, any | null>): LegacyControlValvesModelable;

  mergeJson(json: any): LegacyControlValvesModelable;
   ...
   */
  mergeChildren(deep?: boolean, objKey?: λFunction<Obj | null, any | null>, filter?: λPredicate<string>): LegacyControlValvesModelable;
   ...
   */
  sumObj(other: Obj, deep?: boolean): LegacyControlValvesModelable;
   ...
   */
  singletonArray(): Array_Type<LegacyControlValvesModelable>;

  /**
   * Creates an array of instances of this type.
   */
  static array(...elements: Array_Type<any>[]): Array_Type<LegacyControlValvesModelable> | null;

  /**
   * Creates an array of instances of this type.
   */
  static arrayBuilder(): ArrayBuilder<LegacyControlValvesModelable> | null;
   ...
   * @return new array instance with this as only element.
   */
  singletonSet(): Set_Type<LegacyControlValvesModelable>;

  /**
   * Creates a set of instances of this type.
   */
  static setBuilder(): SetBuilder<LegacyControlValvesModelable> | null;

  /**
   * Create a map of string to elements of this type.
   */
  static mapBuilder(): MapBuilder<string, LegacyControlValvesModelable> | null;

  /**
   * Create a map with the given key type and elements of this type.
   */
  static mapBuilderOf(keyType: ValueType): MapBuilder<any, LegacyControlValvesModelable> | null;

  /**
   * @return new ObjBuilder with initial state set to fields of this instance.
   */
  toBuilder(): ObjBuilder<LegacyControlValvesModelable>;

  /**
   * @return new ObjBuilder of this instance.
   */
  static builder(): ObjBuilder<LegacyControlValvesModelable>;

  /**
   * Construct instance of this type from provided field values and options
   */
  static fromFields(fields: Map_Type<FieldType, any>, spec: Obj.MakeSpec): LegacyControlValvesModelable;
   ...
   * @see withDefaults
   */
  static fromFields(fields: Map_Type<FieldType, any | any>, withDefaults?: boolean): LegacyControlValvesModelable;

  /**
   * Construct instance of this type from provided field values and options
   */
  static make(fields: Map_Type<string, any>, spec: Obj.MakeSpec): LegacyControlValvesModelable;
   ...
   * @see withDefaults
   */
  static make(withDefaults?: boolean): LegacyControlValvesModelable;
   ...
   * @see withDefaults
   */
  static make(fields: Map_Type<string, any | any>, withDefaults?: boolean): LegacyControlValvesModelable;
   ...
   * @see withDefaults
   */
  static make(fields: any, withDefaults?: boolean): LegacyControlValvesModelable;
   ...
   * @see #fromString
   */
  static make(s: string): LegacyControlValvesModelable | null;

  /**
   * Construct an instance of this type from provided instance of a subtype or a "duck type".
   */
  static remake(other: Obj, failIfExtraOrInvalidFields?: boolean): LegacyControlValvesModelable;
   ...
   * Note that it introduces additional overhead so should only be implemented for low volume data.
   */
  afterMake(): LegacyControlValvesModelable;
   ...
   */
  static cachedEmptyInst(): LegacyControlValvesModelable;

  /**
   * Generate a stream of instances of this type. The stream is endless and will call #generateObj each time a new
   * value is read.
   */
  static generateObjs(spec?: Obj.GenerateSpec): Stream<LegacyControlValvesModelable> | null;
   ...
   */
  static generateObj(spec?: Obj.GenerateSpec): LegacyControlValvesModelable;
  ...
  /**
   * Method for training the reconstruction model using the LightGBM algorithm.
   * This function will be used to train the control valve model based on the data provided, and it will leverage the `py-data-science-lightgbm` pipeline.
   *
   * @returns The trained model and associated training metrics.
   */
  doTrainReconstruction(pipe: MlAtomicPipe<any, any, any>, beforeTrainResults: any, spec?: MlOperationSpec): any;

  /**
   * Method for processing the reconstruction model using the LightGBM algorithm.
   * This function processes new data using the trained model, applying it to reconstruct control valve data.
   *
   * @returns The reconstructed data based on the trained model.
   */
  doProcessReconstruction(pipe: MlAtomicPipe<any, any, any>, beforeProcessOutputs: any, spec?: MlOperationSpec): any;

  /**
   * This method should be implemented to specify how to train the
   * {@link ReliabilityRiskMl.ControlValvesReconstructionPipeline#riskScorePipe}, and is a wrapper around
   * the {@link MlAtomicPipe#doTrain} API for the {@link ReliabilityRiskMl.RiskScoreComputation.ControlValvesPipe}.
   *
   * @param pipe
   *        The {@link ReliabilityRiskMl.RiskScoreComputation.ControlValvesPipe} that calls this method.
   * @param beforeTrainResults
   *        Please refer to the {@MlAtomicPipe#doTrain} API for details.
   * @param spec
   *        Please refer to the {@MlAtomicPipe#doTrain} API for details.
   * @return Please refer to the {@MlAtomicPipe#doTrain} API for details.
   */
  doTrainRiskScore(pipe: MlAtomicPipe<any, any, any>, beforeTrainResults: any, spec?: MlOperationSpec): any;

  /**
   * This method should be implemented to specify how to process the
   * {@link ReliabilityRiskMl.ControlValvesReconstructionPipeline#riskScorePipe}, and is a wrapper around
   * the {@link MlAtomicPipe#doProcess} API for the {@link ReliabilityRiskMl.RiskScoreComputation.ControlValvesPipe}.
   *
   * @param pipe
   *        The {@link ReliabilityRiskMl.RiskScoreComputation.ControlValvesPipe} that calls this method.
   * @param beforeProcessOutputs
   *        Please refer to the {@MlAtomicPipe#doProcess} API for details.
   * @param spec
   *        Please refer to the {@MlAtomicPipe#doProcess} API for details.
   * @return Please refer to the {@MlAtomicPipe#doProcess} API for details.
   */
  doProcessRiskScore(pipe: MlAtomicPipe<any, any, any>, beforeProcessOutputs: any, spec?: MlOperationSpec): any;

  /**
   * This method should be implemented to specify how to generate feature contributions for the
   * {@link ReliabilityRiskMl.ControlValvesReconstructionPipeline#interpreter}, and is a wrapper around the
   * {@link ReliabilityRiskMl.Pipeline.Interpreter.ControlValvesInterpreter#doInterpret} API.
   *
   * @param pipe
   *        The trained {@link ReliabilityRiskMl.Reconstruction.ControlValvesPipe} that calls this method.
   * @param x
   *        A native pandas dataframe with the features to interpret.
   * @return A pandas dataframe with the computed feature contributions.
   */
  doInterpret(pipe: MlAtomicPipe<any, any, any>, x: any): any;
   ...
   */
  static fetch(spec?: FetchSpec): FetchResult<LegacyControlValvesModelable>;
   ...
   * @return Requested objs.
   */
  static fetch(filter: Filter): FetchResult<LegacyControlValvesModelable>;
   ...
   */
  static fetchObjStream(spec?: FetchStreamSpec): Stream<LegacyControlValvesModelable> | null;
   ...
   */
  static fetchMultiLocale(spec?: MultiLocaleFetchSpec): Map_Type<string, FetchResult<LegacyControlValvesModelable>> | null;
   ...
   */
  get(include?: string): LegacyControlValvesModelable | null;
   ...
   *         those fields will be returned. Otherwise the entire obj will be returned.
   */
  getSpecific(include?: string): LegacyControlValvesModelable | null;
   ...
   */
  getMissing(spec: GetMissingSpec): LegacyControlValvesModelable;
   ...
   *         those fields will be returned. Otherwise the entire obj will be returned.
   */
  getDirect(include?: string): LegacyControlValvesModelable | null;
   ...
   */
  applyReverseEdit(versionEdit: VersionEdit): LegacyControlValvesModelable;
   ...
   * @see #toString
   */
  static fromString(s: string): LegacyControlValvesModelable | null;
   ...
   */
  create(spec?: UpsertSpec): LegacyControlValvesModelable | null;
   ...
   */
  static createBatch(objs: Array_Type<LegacyControlValvesModelable>, spec?: UpsertSpec): ObjList<LegacyControlValvesModelable> | null;
   ...
   */
  static createBatchObjStream(objs: Stream<LegacyControlValvesModelable>, spec?: CreateBatchObjStreamSpec): ObjList<LegacyControlValvesModelable> | null;
   ...
   */
  update(srcObj?: LegacyControlValvesModelable, spec?: UpsertSpec): LegacyControlValvesModelable | null;
   ...
   */
  upsert(srcObj?: LegacyControlValvesModelable, spec?: UpsertSpec): LegacyControlValvesModelable | null;
   ...
   */
  merge(spec?: MergeSpec): LegacyControlValvesModelable | null;
   ...
   * @return The created or updated obj.
   */
  merge(mergeInclude: string, spec?: MergeSpec): LegacyControlValvesModelable | null;
   ...
   * @return The created or updated obj.
   */
  touch(spec?: TouchSpec): LegacyControlValvesModelable | null;
   ...
   */
  static updateBatch(objs: Array_Type<LegacyControlValvesModelable>, srcObjs?: Array_Type<LegacyControlValvesModelable>, spec?: UpsertSpec): ObjList<LegacyControlValvesModelable> | null;
   ...
   *         populated. Otherwise only the id field will be populated.
   */
  static updateObjs(objs: λSupplier<Array_Type<LegacyControlValvesModelable> | null>, spec?: UpsertSpec): ObjList<LegacyControlValvesModelable> | null;
   ...
   *         fields populated. Otherwise only the id field will be populated.
   */
  static upsertBatch(objs: Array_Type<LegacyControlValvesModelable>, srcObjs?: Array_Type<LegacyControlValvesModelable>, spec?: UpsertSpec): ObjList<LegacyControlValvesModelable> | null;
   ...
   *         fields populated. Otherwise only the id field will be populated.
   */
  static upsertObjs(objs: λSupplier<Array_Type<LegacyControlValvesModelable> | null>, spec?: UpsertSpec): ObjList<LegacyControlValvesModelable> | null;
   ...
   *         populated. Otherwise only the id field will be populated.
   */
  static touchBatch(objs: Array_Type<LegacyControlValvesModelable>, spec?: TouchSpec): ObjList<LegacyControlValvesModelable> | null;
   ...
   * @see Mergespec#csvInclude
   */
  static mergeBatch(objs: Array_Type<LegacyControlValvesModelable>, spec?: MergeSpec): ObjList<LegacyControlValvesModelable> | null;
   ...
   * @see Mergespec#csvInclude
   */
  static mergeBatch(objs: Array_Type<LegacyControlValvesModelable>, mergeInclude: string, spec?: MergeSpec): ObjList<LegacyControlValvesModelable> | null;
   ...
   * @see Mergespec#csvInclude
   */
  static mergeObjs(objs: λSupplier<Array_Type<LegacyControlValvesModelable> | null>, mergeInclude: string, spec?: MergeSpec): ObjList<LegacyControlValvesModelable> | null;
   ...
   */
  static mergeAll(mergeObj: LegacyControlValvesModelable, spec?: MergeAllSpec): number | null;
   ...
   * @return The number of objs that were merged.
   */
  static mergeAll(mergeObj: LegacyControlValvesModelable, mergeInclude: string, spec?: MergeAllSpec): number | null;
   ...
   * @see Ann.Db#archive
   */
  static removeBatch(objs: Array_Type<LegacyControlValvesModelable>, spec?: UpsertSpec): ObjList<LegacyControlValvesModelable> | null;
   ...
   *         populated. Otherwise only the id field will be populated.
   */
  static replace(objs: Array_Type<LegacyControlValvesModelable>, spec?: UpsertSpec): ObjList<LegacyControlValvesModelable> | null;
   ...
   * @see Ann.Db#archive
   */
  unremove(): LegacyControlValvesModelable | null;
   ...
   */
  static beforeCreate(objs: Array_Type<LegacyControlValvesModelable>): ObjList<LegacyControlValvesModelable>;
   ...
   *         of objs must match the input.
   */
  static beforeUpdate(objs: Array_Type<LegacyControlValvesModelable>): ObjList<LegacyControlValvesModelable>;
   ...
   *         of objs must match the input.
   */
  static beforeRemove(objs: Array_Type<LegacyControlValvesModelable>): ObjList<LegacyControlValvesModelable>;
   ...
   */
  static afterCreate(objs: Array_Type<LegacyControlValvesModelable>): Array_Type<ObjError> | null;
   ...
   * @return List of any errors that were encountered.
   */
  static afterUpdate(objs: Array_Type<LegacyControlValvesModelable>): Array_Type<ObjError> | null;
   ...
   * @return List of any errors that were encountered.
   */
  static afterRemove(objs: Array_Type<LegacyControlValvesModelable>): Array_Type<ObjError> | null;
   ...
   */
  static startImportData(spec?: StartImportDataSpec): PushStream<LegacyControlValvesModelable>;

  /**
   * @return a push stream to import instances of this type.
   */
  static startImportDataWithStats(spec?: StartImportDataSpec): PushStreamWithStats<LegacyControlValvesModelable>;
   ...
   */
  static forId(id: string, failIfMissing?: boolean): LegacyControlValvesModelable | null;
   ...
   */
  withoutIdentity(): LegacyControlValvesModelable | null;
   ...
   */
  static typeOfBatch(objs: Array_Type<LegacyControlValvesModelable>): Array_Type<EntityType> | null;
   ...
   */
  static dbEcho(template?: LegacyControlValvesModelable, count?: number, sendBack?: boolean): number | null;
   ...
   */
  static callbackLogic(objs: Array_Type<LegacyControlValvesModelable>, callback?: λFunction<LegacyControlValvesModelable | null, LegacyControlValvesModelable | null>): ObjList<LegacyControlValvesModelable>;

  /**
   * Implements a simple logic for Persistable "after" call-backs like after create by looping each input obj and
   * calling a transform for it.
   */
  static afterCallbackLogic(objs: Array_Type<LegacyControlValvesModelable>, callback?: λFunction<LegacyControlValvesModelable | null, LegacyControlValvesModelable | null>): Array_Type<ObjError> | null;
  ...
  /**
   * Returns the list of tree-based regressors that are supported and can be used to generate reconstructions using
   * this approach, such that feature contributions can be generated using SHAP from the {@link doInterpret} function.
   * Users can also used linear models to generate reconstructions.
   */
  static getSupportedRegressors(): Array_Type<string>;
}
